...
async def handle_list_tools() -> list[types.Tool]:
    ...
    return [
        ...
        types.Tool(
            name="ui_ux_analysis",
            description="Run a UI/UX analysis using the ui_ux_analysis agent with sequential_thinking and playwright tools.",
            inputSchema={
                "type": "object",
                "properties": {
                    "message": {
                        "type": "string",
                        "description": "The UI/UX analysis prompt or question."
                    }
                },
                "required": ["message"]
            }
...
async def run_ui_ux_analysis(arguments: dict) -> list[types.TextContent]:
    message = arguments.get("message")
    if not message:
        raise ValueError("Missing message parameter")
    agent = Agent(
        name="ui_ux_analysis",
        instruction="Perform a UI/UX analysis using sequential thinking, Playwright, and Filesystem tools.",
        server_names=["sequential_thinking", "playwright", "filesystem"]
    )
    await agent.initialize()
    llm = await agent.attach_llm(OpenAIAugmentedLLM)
...
async def handle_call_tool(
    ...
    try:
        ...
        elif name == "ui_ux_analysis":
            return await run_ui_ux_analysis(arguments)
        else:
