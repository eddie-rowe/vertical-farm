...
import { CheckCircle, BarChart2, Settings, Zap, Leaf, Users, BookOpen, Rss } from 'lucide-react';
    ...
    <nav className="w-full p-4 fixed top-0 left-0 bg-white dark:bg-gray-900 shadow-md z-50">
      <div className="container mx-auto flex justify-between items-center">
        <Link href="/" passHref className="flex items-center gap-2">
          ...
          <span className="font-semibold text-xl text-green-700 dark:text-green-300">VerticalFarm OS</span>
        </Link>
        <div className="space-x-2 sm:space-x-4">
          <Link href="/#features" passHref className="text-gray-600 dark:text-gray-300 hover:text-green-600 dark:hover:text-green-400 text-sm sm:text-base">
            Features
          </Link>
          <Link href="/#audience" passHref className="text-gray-600 dark:text-gray-300 hover:text-green-600 dark:hover:text-green-400 text-sm sm:text-base">
            For Who?
          </Link>
...
const FeatureCard = ({ icon, title, description }: { icon: React.ReactNode, title: string, description: string }) => (
  <Card className="bg-white/70 dark:bg-gray-800/70 backdrop-blur-sm shadow-lg hover:shadow-xl transition-shadow duration-300 glass">
    <CardHeader className="flex flex-row items-center gap-4 pb-2">
      {icon}
      <CardTitle className="text-green-800 dark:text-green-300">{title}</CardTitle>
    </CardHeader>
    <CardContent>
  ...
  return (
    <>
      ...
      <div className="min-h-screen w-full flex flex-col items-center justify-center gradient-farm animate-pop pt-20 sm:pt-16">
        <main className="flex flex-col items-center justify-center flex-1 w-full px-4 py-12 sm:py-16 text-center">
          <div className="glass card-shadow max-w-2xl w-full flex flex-col items-center gap-6 sm:gap-8 p-6 sm:p-12 animate-pop">
            ...
            <p className="text-lg sm:text-xl text-center text-gray-700 dark:text-gray-300 max-w-lg">
              Take control of your vertical farm with VerticalFarm OS: model layouts, automate grow recipes, track yields, and gain actionable insights. All powered by Home Assistant for local-first privacy.
            </p>
            <Link href="/login" passHref>
              <Button size="lg" variant="default" className="px-10 py-6 text-xl font-semibold shadow-xl hover:shadow-2xl transition-shadow duration-300 mt-4">
                Start Growing
              </Button>
            </Link>
          </div>
        ...
        <section id="features" className="w-full py-16 sm:py-24 bg-white dark:bg-gray-800/50 backdrop-blur-md">
          <div className="container mx-auto text-center px-4">
            <h2 className="text-3xl sm:text-4xl font-bold mb-12 sm:mb-16 text-green-800 dark:text-green-200">Everything You Need to Grow</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
              <FeatureCard
                icon={<Settings className="w-10 h-10 text-green-600 dark:text-green-400" />}
              ...
              />
              <FeatureCard
                icon={<BarChart2 className="w-10 h-10 text-green-600 dark:text-green-400" />}
        ...
        <section id="audience" className="w-full py-16 sm:py-24 bg-gray-50 dark:bg-gray-900/60 backdrop-blur-md">
          <div className="container mx-auto text-center px-4">
            <h2 className="text-3xl sm:text-4xl font-bold mb-12 sm:mb-16 text-green-800 dark:text-green-200">Built for Every Grower</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8">
              <div className="p-6 rounded-lg glass card-shadow_light">
                ...
                <h3 className="text-xl font-semibold mb-2 text-green-800 dark:text-green-300">Hobbyists</h3>
                <p className="text-sm text-gray-600 dark:text-gray-300">Simple setup, visual plant health, and blueprint templates for one-click automations.</p>
              </div>
              <div className="p-6 rounded-lg glass card-shadow_light">
                <Leaf className="w-12 h-12 text-green-600 dark:text-green-400 mx-auto mb-4" />
                <h3 className="text-xl font-semibold mb-2 text-green-800 dark:text-green-300">Urban Farms</h3>
                <p className="text-sm text-gray-600 dark:text-gray-300">Achieve consistent yields and optimize resource usage with centralized logs and dashboards.</p>
              </div>
              <div className="p-6 rounded-lg glass card-shadow_light">
                <Settings className="w-12 h-12 text-green-600 dark:text-green-400 mx-auto mb-4" />
                <h3 className="text-xl font-semibold mb-2 text-green-800 dark:text-green-300">AgTech Developers</h3>
                <p className="text-sm text-gray-600 dark:text-gray-300">Rapidly prototype with modular APIs and publish new device drivers or ML controls.</p>
              </div>
              <div className="p-6 rounded-lg glass card-shadow_light">
                <BookOpen className="w-12 h-12 text-green-600 dark:text-green-400 mx-auto mb-4" />
                <h3 className="text-xl font-semibold mb-2 text-green-800 dark:text-green-300">Researchers</h3>
                <p className="text-sm text-gray-600 dark:text-gray-300">Access open data, run reproducible experiments, and share full farm-control stacks.</p>
              </div>
            </div>
          </div>
        </section>
        ...
        <footer className="w-full flex flex-col items-center gap-4 py-8 text-gray-600 dark:text-gray-200 text-sm bg-gray-100 dark:bg-gray-950 border-t border-gray-200 dark:border-gray-800">
          <div className="flex gap-4 sm:gap-6">
            <a href="https://github.com/eddie-rowe/vertical-farm" target="_blank" rel="noopener noreferrer" className="footer-link flex items-center gap-1">
            ...
            <a href="https://eddie-rowe.github.io/vertical-farm/" target="_blank" rel="noopener noreferrer" className="footer-link flex items-center gap-1">
              <BookOpen className="w-5 h-5" /> Docs
            </a>
            <a href="https://github.com/eddie-rowe/vertical-farm/issues" target="_blank" rel="noopener noreferrer" className="footer-link flex items-center gap-1">
              <Rss className="w-5 h-5" /> Report an Issue
            </a>
          </div>
          <div className="font-medium">© {new Date().getFullYear()} VerticalFarm OS. All rights reserved.</div>
        </footer>
      </div>
    </>
  );
