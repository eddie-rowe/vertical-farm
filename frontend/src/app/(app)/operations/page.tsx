...
export default function OperationsPage() {
  ...
  const getStatusColor = (status: EfficiencyMetric['status']) => {
    switch (status) {
      ...
      case 'warning': return 'state-maintenance';
      case 'critical': return 'state-offline';
      default: return 'text-control-label';
    }
  };
  ...
  const getTrendColor = (trend: 'up' | 'down' | 'stable') => {
    switch (trend) {
      case 'up': return 'state-offline';
      case 'down': return 'state-growing';
      case 'stable': return 'text-control-label';
    }
  };
    ...
    <div className="container mx-auto p-6 space-y-6">
      ...
      >
        <div className="flex space-x-2">
          <FarmControlButton variant="default" animation="float">Export Report</FarmControlButton>
          <FarmControlButton variant="primary" animation="pop">Generate Analysis</FarmControlButton>
        </div>
      </PageHeader>
      ...
      <Tabs defaultValue="efficiency" className="space-y-6">
        ...
        <TabsContent value="efficiency" className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {efficiencyMetrics.map((metric) => (
              <Card key={metric.id} className="hover:shadow-lg transition-shadow card-shadow">
                ...
                <CardContent>
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center space-x-1 text-control-label">
                      ...
                      <span className={metric.change >= 0 ? 'state-growing' : 'state-offline'}>
        ...
        <TabsContent value="costs" className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card className="card-shadow">
              <CardHeader>
                <CardTitle className="text-farm-title">Monthly Cost Breakdown</CardTitle>
                <CardDescription className="text-control-label">Operating costs by category for current month</CardDescription>
              </CardHeader>
              <CardContent>
                  ...
                  {costBreakdown.map((cost, index) => (
                    <div key={index} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                      <div className="flex items-center space-x-3">
                        {cost.icon}
                        <div>
                          <p className="text-control-label font-medium">{cost.category}</p>
                          <p className="text-control-label opacity-70">{cost.percentage}% of total</p>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="text-sensor-value">${cost.amount}</p>
                        <p className={`text-control-label ${getTrendColor(cost.trend)}`}>
            ...
            <Card className="card-shadow">
              <CardHeader>
                ...
                <CardDescription className="text-control-label">Historical cost analysis and projections</CardDescription>
              </CardHeader>
              <CardContent>
                <ResourceUsageChart
                  data={[
        ...
        <TabsContent value="performance" className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card className="card-shadow">
              <CardHeader>
                <CardTitle className="text-farm-title">Performance vs Targets</CardTitle>
                <CardDescription className="text-control-label">Current month performance against set targets</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  {performanceData.map((item, index) => (
                    <div key={index} className="space-y-2">
                      <div className="flex justify-between items-center">
                        <span className="text-control-label font-medium">{item.metric}</span>
                        <span className="text-control-label">
                      ...
                      <div className="flex justify-between items-center text-control-label">
                        <span className={`font-medium ${item.achievement >= 100 ? 'state-growing' : item.achievement >= 90 ? 'state-maintenance' : 'state-offline'}`}>
            ...
            <Card className="card-shadow">
              <CardHeader>
                ...
                <CardDescription className="text-control-label">Return on investment and profitability metrics</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
