...
export const checkAccessibility = async (container: HTMLElement) => {
  ...
  const checks = {
    ...
    hasHeadings:
      container.querySelectorAll("h1, h2, h3, h4, h5, h6").length > 0,
    hasSkipLinks:
      container.querySelectorAll('a[href="#main"], a[href="#content"]').length >
      0,
  };
  ...
  return {
    violations: Object.entries(checks)
      ...
      .map(([rule]) => ({
        id: rule,
        description: `Accessibility rule ${rule} failed`,
      })),
    passes: Object.entries(checks)
      .filter(([, passed]) => passed)
      .map(([rule]) => ({
        id: rule,
        description: `Accessibility rule ${rule} passed`,
      })),
  };
};
