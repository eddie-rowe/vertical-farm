...
export const FanSchema = z.object({
  ...
  airflow_cfm: z.number().positive("Airflow must be positive.").optional().nullable(),
  power_watts: z.number().positive("Power must be positive.").optional().nullable(),
  ...
  orientation: z.enum(['up', 'down', 'front', 'back', 'left', 'right']).optional().nullable(),
});
