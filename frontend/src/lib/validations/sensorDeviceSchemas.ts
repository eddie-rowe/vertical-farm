...
export const SensorDeviceSchema = z
  .object({
  ...
  .refine(
    (data) => {
      if (
        typeof data.data_range_min === "number" &&
        typeof data.data_range_max === "number"
      ) {
    ...
    {
      message:
        "Max data range must be greater than or equal to min data range when both are specified",
      path: ["data_range_max"],
    },
  );
