...
export const siteConfig: SiteConfig = {
  name: "Vertical Farm",
  description: "Advanced vertical farming management system with IoT integration and Home Assistant compatibility",
...
export const structuredDataGenerators = {
  website: () => generateStructuredData({
    type: 'WebSite',
    data: {
  ...
  breadcrumbs: (items: Array<{ name: string; url: string }>) => generateStructuredData({
    type: 'BreadcrumbList',
    data: {
...
export function generateSocialPreviewTags(config: SEOConfig) {
  ...
  if (config.description) tags.push({ property: 'og:description', content: config.description })
  ...
  // Twitter
  if (config.twitterCard) tags.push({ name: 'twitter:card', content: config.twitterCard })
  if (config.twitterSite) tags.push({ name: 'twitter:site', content: config.twitterSite })
  if (config.twitterCreator) tags.push({ name: 'twitter:creator', content: config.twitterCreator })
...
export const seoHelpers = {
  ...
  generatePreloadHints: (resources: Array<{ href: string; as: string; type?: string }>) => {
  ...
  validateContent: (content: { title?: string; description?: string; headings?: string[] }) => {
    const issues = []
