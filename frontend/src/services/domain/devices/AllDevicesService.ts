...
export interface UnifiedDevice {
  ...
  // Integration source
  integration_type: 'home_assistant' | 'smartthings' | 'arduino' | 'aws_iot' | 'raspberry_pi';
...
export class AllDevicesService extends BaseService {
   ...
   */
  async getFilteredDevices(filters: AllDevicesFilter = {}): Promise<UnifiedDevice[]> {
    ...
    return ErrorHandler.withErrorHandling(async () => {
      ...
      if (filters.integration_type && filters.integration_type !== 'all') {
        devices = devices.filter(device => device.integration_type === filters.integration_type);
      }
      
      if (filters.device_type && filters.device_type !== 'all') {
        devices = devices.filter(device => 
          device.domain === filters.device_type || device.device_class === filters.device_type
        );
      }
      ...
      if (filters.status && filters.status !== 'all') {
        devices = devices.filter(device => {
          switch (filters.status) {
            case 'online':
              return device.is_online !== false && device.state !== 'unavailable';
            case 'offline':
  ...
  async getDeviceStats(): Promise<DeviceStats> {
    ...
    return ErrorHandler.withErrorHandling(async () => {
      ...
      devices.forEach(device => {
        ...
        stats.by_integration[integrationName] = (stats.by_integration[integrationName] || 0) + 1;
   ...
   */
  async controlDevice(device: UnifiedDevice, action: 'turn_on' | 'turn_off' | 'toggle'): Promise<void> {
    ...
    return ErrorHandler.withErrorHandling(async () => {
      switch (device.integration_type) {
        ...
        default:
          throw new Error(`Device control not supported for integration: ${device.integration_type}`);
      }
  ...
  async importDevice(device: UnifiedDevice): Promise<void> {
    ...
    return ErrorHandler.withErrorHandling(async () => {
      switch (device.integration_type) {
        ...
        default:
          throw new Error(`Device import not supported for integration: ${device.integration_type}`);
      }
  ...
  private async getHomeAssistantDevices(): Promise<UnifiedDevice[]> {
    try {
      ...
      const assignmentService = (await import('@/services/deviceAssignmentService')).default;
  ...
  async bulkImportDevices(devices: UnifiedDevice[]): Promise<void> {
    ...
    return ErrorHandler.withErrorHandling(async () => {
      ...
      const devicesByIntegration = devices.reduce((acc, device) => {
      ...
      for (const [integrationType, integrationDevices] of Object.entries(devicesByIntegration)) {
        switch (integrationType) {
          ...
          default:
            console.warn(`Bulk import not supported for integration: ${integrationType}`);
        }
      }
    }, 'Bulk import devices');
  }
   ...
   */
  async bulkControlDevices(devices: UnifiedDevice[], action: 'turn_on' | 'turn_off'): Promise<void> {
    ...
    return ErrorHandler.withErrorHandling(async () => {
      ...
      for (const [integrationType, integrationDevices] of Object.entries(devicesByIntegration)) {
        switch (integrationType) {
          ...
          default:
            console.warn(`Bulk control not supported for integration: ${integrationType}`);
        }
      }
