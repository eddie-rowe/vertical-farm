  ...
  recordFailure(): void {
    ...
    if (this.state === CircuitState.CLOSED) {
      if (this.failureCount >= this.config.failureThreshold) {
        ...
        this.nextAttemptTime = new Date(Date.now() + this.config.recoveryTimeout * 1000);
...
export class ErrorHandler {
  ...
  createEnhancedError(
    ...
    const errorType = this.classifyError(error instanceof Error ? error : new Error(message));
    const retryConfig = this.retryConfigs.get(serviceName || 'default');
    const retryable = retryConfig ? retryConfig.retryableErrors.includes(errorType) : false;

    return new EnhancedError(message, errorType, retryable, context, originalError);
  }
  ...
  shouldRetry(serviceName: string, error: EnhancedError, attempt: number): boolean {
    const config = this.retryConfigs.get(serviceName);
  ...
  private showUserError(error: EnhancedError, serviceName: string): void {
    const userFriendlyMessages: Record<ErrorType, string> = {
      [ErrorType.CONNECTION_ERROR]: 'Unable to connect to Home Assistant. Please check your connection.',
      ...
      [ErrorType.SERVICE_UNAVAILABLE]: 'Home Assistant service is temporarily unavailable.',
    ...
    if (!error.retryable || error.context.attempt === error.context.maxAttempts) {
  ...
  getServiceHealth(serviceName: string): any {
    ...
    return {
      status: circuitBreaker.getState() === CircuitState.CLOSED ? 'healthy' : 'unhealthy',
...
globalErrorHandler.registerService('home_assistant_api', {
  maxAttempts: 3,
  baseDelay: 1000,
  maxDelay: 30000,
  exponentialBase: 2,
...
globalErrorHandler.registerService('websocket', {
  maxAttempts: 5,
  baseDelay: 2000,
  maxDelay: 60000,
  exponentialBase: 2,
