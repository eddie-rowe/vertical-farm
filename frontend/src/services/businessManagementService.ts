...
class BusinessManagementService {
    ...
    try {
      ...
      orders.forEach((order) => {
        ...
        customerSpending.set(
          customerId,
          (customerSpending.get(customerId) || 0) + amount / 100,
        );
      ...
      return squareCustomers.slice(0, limit).map((customer) => ({
        ...
        lastOrder: customerLastOrders.get(customer.id)
          ? new Date(customerLastOrders.get(customer.id)!)
              .toISOString()
              .split("T")[0]
          : undefined,
    ...
    try {
      ...
      return squareOrders.slice(0, limit).map((order) => {
        ...
        return {
          ...
          dueDate: invoice?.scheduled_at
            ? new Date(invoice.scheduled_at).toISOString().split("T")[0]
            : undefined,
  ...
  async getPayments(limit: number = 50): Promise<BusinessPayment[]> {
    try {
      ...
      refunds.forEach((refund) => {
        const existing = refundMap.get(refund.payment_id) || {
          amount: 0,
          count: 0,
        };
      ...
      return squarePayments.slice(0, limit).map((payment) => {
        ...
        return {
          ...
          netAmount:
            this.getPaymentAmount(payment) -
            (this.calculateProcessingFee(payment) || 0),
  ...
  async getInventory(limit: number = 25): Promise<BusinessInventoryItem[]> {
    try {
      ...
      const inventoryItems: BusinessInventoryItem[] = inventoryData
        .slice(0, limit)
        .map((item) => {
          ...
          return {
            ...
            status:
              parseInt(item.quantity || "0") > 0 ? "Active" : "Out of Stock",
            lastUpdated: item.calculated_at
              ? new Date(item.calculated_at).toISOString().split("T")[0]
              : new Date().toISOString().split("T")[0],
          };
        });
  ...
  async getRefunds(limit: number = 20): Promise<BusinessRefund[]> {
    try {
      ...
      return refunds.slice(0, limit).map((refund) => ({
        ...
        customer: this.getRefundCustomerName(
          refund,
          paymentMap,
          orderMap,
          customerMap,
  ...
  async getDisputes(limit: number = 10): Promise<BusinessDispute[]> {
    try {
      ...
      return disputes.slice(0, limit).map((dispute) => ({
        ...
        dueDate: dispute.due_at
          ? new Date(dispute.due_at).toISOString().split("T")[0]
          : undefined,
  ...
  async getSubscriptions(limit: number = 15): Promise<BusinessSubscription[]> {
    try {
      ...
      return subscriptions.slice(0, limit).map((subscription) => ({
        ...
        customer:
          customerMap.get(subscription.customer_id || "")?.company_name ||
  ...
  async getInvoices(limit: number = 20): Promise<BusinessInvoice[]> {
    try {
      ...
      return invoices.slice(0, limit).map((invoice) => ({
        ...
        customer:
          customerMap.get(invoice.primary_recipient.customer_id || "")
            ?.company_name ||
        ...
        paymentMethods: this.getInvoicePaymentMethods(
  ...
  async getTeamMembers(limit: number = 10): Promise<BusinessTeamMember[]> {
    try {
      ...
      return teamMembers.slice(0, limit).map((member) => ({
        ...
        locations:
          member.assigned_locations?.location_ids?.map(
            (id) => locationMap.get(id)?.name || id,
          ) || [],
    ...
    try {
      ...
      return laborEntries.slice(0, limit).map((entry) => {
        ...
        const duration = endTime
          ...
          : null;
        const hourlyRate = entry.wage?.hourly_rate?.amount
          ? entry.wage.hourly_rate.amount / 100
          : 0;
        ...
        return {
          ...
          employeeName: member
            ...
            : "Unknown Employee",
          location:
            locationMap.get(entry.location_id)?.name || "Unknown Location",
          startTime: startTime.toLocaleString(),
          endTime: endTime?.toLocaleString(),
          duration: duration ? `${duration} hours` : "In progress",
          hourlyRate,
          totalPay:
            duration && hourlyRate ? parseFloat(duration) * hourlyRate : 0,
          status: entry.status,
  ...
  async getPayouts(limit: number = 15): Promise<BusinessPayout[]> {
    try {
      ...
      return payouts.slice(0, limit).map((payout) => ({
        ...
        location:
          locationMap.get(payout.location_id)?.name || "Unknown Location",
  ...
  private getPaymentCustomerName(
    payment: any,
  ...
  private getRefundCustomerName(
    refund: any,
    paymentMap: Map<string, any>,
    orderMap: Map<string, any>,
    customerMap: Map<string, any>,
  ): string {
  ...
  private mapOrderItems(
    lineItems: any[],
  ): Array<{ product: string; quantity: number; unit: string; price: number }> {
  ...
  async getRevenueAnalytics() {
    try {
      ...
      const processingFees = payments.reduce(
        (sum, payment) =>
          payment.status === "Paid" && payment.processingFee
