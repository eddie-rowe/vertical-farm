...
function stateManagerReducer(state: StateManagerState, action: StateManagerAction): StateManagerState {
  switch (action.type) {
    case 'SET_FARM_DATA': {
      ...
      const newHistory = JSON.stringify(state.current) !== JSON.stringify(action.payload)
    ...
    case 'UNDO': {
      ...
      return {
        ...
        history: newHistory,
        future: [currentSnapshot, ...state.future].slice(0, state.maxHistorySize),
        isDirty: true
      };
    }
    ...
    case 'REDO': {
      ...
      return {
        ...
        history: [...state.history, currentSnapshot].slice(-state.maxHistorySize),
        future: newFuture,
        isDirty: true
      };
    }
