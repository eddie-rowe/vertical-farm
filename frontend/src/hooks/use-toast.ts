...
export const reducer = (state: State, action: Action): State => {
  switch (action.type) {
    ...
    case "DISMISS_TOAST": {
      ...
      // Add to remove queue if toastId is provided
      if (toastId) {
...
interface ToastOptions {
  /**
   * Duration in milliseconds. Set to 0 to disable auto-dismiss.
   * @default 5000
   */
  duration?: number
  /**
   * Whether the toast should be persistent (not auto-dismiss)
   * @default false
   */
  persistent?: boolean
}

function toast({ duration = 5000, persistent = false, ...props }: Toast & ToastOptions) {
  const id = genId()
