...
interface UseGrowAutomationReturn {
  ...
  updateSchedule: (scheduleId: string, scheduleData: Partial<CreateAutomationSchedule>) => Promise<boolean>;
  deleteSchedule: (scheduleId: string) => Promise<boolean>;
  createCondition: (conditionData: CreateAutomationCondition) => Promise<boolean>;
  updateCondition: (conditionId: string, conditionData: Partial<CreateAutomationCondition>) => Promise<boolean>;
  deleteCondition: (conditionId: string) => Promise<boolean>;
  manualExecute: (deviceAssignmentId: string, action: DeviceAction, reason?: string) => Promise<boolean>;
  refreshData: () => Promise<void>;
  
  // Utilities
  getSchedulesByDevice: (deviceAssignmentId: string) => GrowAutomationSchedule[];
  getConditionsByDevice: (deviceAssignmentId: string) => GrowAutomationCondition[];
...
export function useGrowAutomation({ 
  ...
  const [deviceAssignments, setDeviceAssignments] = useState<GrowDeviceAssignment[]>([]);
  ...
  const fetchAutomationData = useCallback(async () => {
    ...
    try {
      ...
      if (!automationResponse.ok) {
        throw new Error(`Failed to fetch automation data: ${automationResponse.statusText}`);
      }
  ...
  const toggleAutomation = useCallback(async (enabled: boolean): Promise<boolean> => {
    if (!growId) return false;
  ...
  const createSchedule = useCallback(async (scheduleData: CreateAutomationSchedule): Promise<boolean> => {
    if (!growId) return false;
  ...
  const createCondition = useCallback(async (conditionData: CreateAutomationCondition): Promise<boolean> => {
    if (!growId) return false;
  ...
  const manualExecute = useCallback(async (
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to execute action manually';
      setError(errorMessage);
  ...
  // Utility functions
  const getSchedulesByDevice = useCallback((deviceAssignmentId: string): GrowAutomationSchedule[] => {
    return schedules.filter(schedule => schedule.device_assignment_id === deviceAssignmentId);
  }, [schedules]);

  const getConditionsByDevice = useCallback((deviceAssignmentId: string): GrowAutomationCondition[] => {
    return conditions.filter(condition => condition.device_assignment_id === deviceAssignmentId);
  }, [conditions]);

  const getRecentExecutions = useCallback((limit: number = 10): GrowAutomationExecution[] => {
    return executions
      .sort((a, b) => new Date(b.executed_at).getTime() - new Date(a.executed_at).getTime())
      .slice(0, limit);
  }, [executions]);

  const isDeviceAutomated = useCallback((deviceAssignmentId: string): boolean => {
    const hasSchedules = schedules.some(s => s.device_assignment_id === deviceAssignmentId && s.is_active);
    const hasConditions = conditions.some(c => c.device_assignment_id === deviceAssignmentId && c.is_active);
    const hasRules = rules.some(r => r.device_assignment_id === deviceAssignmentId && r.is_active);
