...
export async function middleware(req: NextRequest) {
  ...
  if (!isAuthenticated && !PUBLIC_PATHS.includes(pathname) && !PUBLIC_FILE.test(pathname)) {
    const loginUrl = req.nextUrl.clone();
    loginUrl.pathname = '/login';
    if (pathname !== '/') { // Avoid adding redirect for the home page if it's the one being protected
