...
import { DeviceLayerState, DeviceData, DeviceAction, DeviceWebSocketMessage, LocationDevices } from '@/types/device-layer';
...
type DeviceLayerAction =
  ...
  | { type: 'UPDATE_LOCATION_DEVICES'; locationId: string; devices: DeviceData[] }
...
// Reducer
function deviceLayerReducer(state: DeviceLayerState, action: DeviceLayerAction): DeviceLayerState {
  switch (action.type) {
    case 'SET_ENABLED':
    ...
    case 'UPDATE_DEVICES':
      const devicesById = action.devices.reduce((acc, device) => {
...
export function DeviceProvider({ children }: DeviceProviderProps) {
  ...
  const connectWebSocket = useCallback(async () => {
    ...
    try {
      ...
      dispatch({ type: 'SET_CONNECTION_STATUS', status: 'disconnected' });
      dispatch({ type: 'SET_ERROR', error: 'Failed to connect to device service' });
    }
  ...
  const handleWebSocketMessage = useCallback((message: DeviceWebSocketMessage) => {
    switch (message.type) {
  ...
  // API calls
  const loadLocationDevices = useCallback(async (locationId: string) => {
    ...
    try {
      ...
      dispatch({ type: 'UPDATE_LOCATION_DEVICES', locationId, devices: data.devices });
  ...
  const controlDevice = useCallback(async (entityId: string, action: DeviceAction) => {
    if (!session?.access_token) return;
    ...
    try {
      ...
      toast.error(`Device control failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
      throw error;
    }
  ...
  // Utilities
  const getDevicesByLocation = useCallback((locationId: string): DeviceData[] => {
    return state.locationDevices[locationId]?.devices || [];
  }, [state.locationDevices]);

  const getDeviceByEntityId = useCallback((entityId: string): DeviceData | undefined => {
    return state.devices[entityId];
  }, [state.devices]);

  const isDeviceOnline = useCallback((entityId: string): boolean => {
  ...
  useEffect(() => {
    if (state.enabled && state.connectionStatus === 'disconnected' && user && session) {
      connectWebSocket();
    }
