...
export interface UpdateAutomationSchedule
  extends Partial<CreateAutomationSchedule> {
  id: string;
}

export interface UpdateAutomationCondition
  extends Partial<CreateAutomationCondition> {
  id: string;
}
...
export function validateAutomationSchedule(
  schedule: CreateAutomationSchedule,
): string[] {
  const errors: string[] = [];
...
export function validateAutomationCondition(
  condition: CreateAutomationCondition,
): string[] {
  const errors: string[] = [];
  ...
  if (
    ["above", "below", "equals"].includes(condition.condition_type) &&
  ...
  if (
    condition.condition_type === "between" &&
    (condition.threshold_min === undefined ||
      condition.threshold_max === undefined)
  ) {
    errors.push(
      "Both min and max thresholds are required for between conditions",
    );
  }
