  ...
  switch (type) {
    case "schedule":
      return (
        <Clock
          {...iconProps}
          className={`w-4 h-4 ${status === "active" ? "text-blue-500" : "text-gray-400"}`}
        />
      );
    case "condition":
      return (
        <Target
          {...iconProps}
          className={`w-4 h-4 ${status === "triggered" ? "text-orange-500" : status === "active" ? "text-green-500" : "text-gray-400"}`}
        />
      );
    case "sensor":
      return (
        <Thermometer
          {...iconProps}
          className={`w-4 h-4 ${status === "triggered" ? "text-red-500" : status === "active" ? "text-green-500" : "text-gray-400"}`}
        />
      );
    default:
      return (
        <Settings
          {...iconProps}
          className={`w-4 h-4 ${status === "active" ? "text-green-500" : "text-gray-400"}`}
        />
      );
  }
};
        ...
        <TooltipTrigger asChild>
          <div
            className={cn(
              ...
              status === "triggered" &&
                "ring-2 ring-orange-400/50 animate-pulse",
            )}
          >
          ...
          <TooltipContent>
            <div className="text-sm">
              <div className="font-medium">
                {activeRules} automation rules active
              </div>
              <div className="text-xs text-gray-500">
                Click to manage automation
              </div>
            </div>
          </TooltipContent>
        </Tooltip>
