...
export function MonitoringOverlay({
  ...
  const overallHealth =
    allData.length > 0
      ? allData.reduce((sum, data) => sum + data.healthScore, 0) /
        allData.length
      : 0;
  const totalCriticalAlerts = allData.reduce(
    (sum, data) => sum + data.alerts.critical,
    0,
  );
  const totalWarnings = allData.reduce(
    (sum, data) => sum + data.alerts.warnings,
    0,
  );
      ...
      ? [
          {
            ...
            value:
              allData.reduce(
                (sum, data) => sum + data.environmentalData.temperature,
                0,
              ) / allData.length,
            unit: "°C",
          ...
          },
          {
            ...
            value:
              allData.reduce(
                (sum, data) => sum + data.environmentalData.humidity,
                0,
              ) / allData.length,
            unit: "%",
            status: "optimal" as const,
          ...
          },
          {
            ...
            value:
              allData.reduce(
                (sum, data) => sum + data.environmentalData.ph,
                0,
              ) / allData.length,
            unit: "",
          ...
          },
          {
            ...
            value:
              allData.reduce(
                (sum, data) => sum + data.environmentalData.light,
                0,
              ) / allData.length,
            unit: "%",
            status: "optimal" as const,
