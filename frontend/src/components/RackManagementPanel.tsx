...
export default function RackManagementPanel({ racks, onRacksReorder }: RackManagementPanelProps) {
  ...
  const [items, setItems] = React.useState<string[]>(racks.map((rack) => rack.id));
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    if (over && String(active.id) !== String(over.id)) {
      ...
      setItems(newItems);
      if (onRacksReorder) {
        const newRacks = newItems.map((id) => racks.find((rack) => rack.id === id) as Rack);
  ...
  return (
    <section className="p-4 rounded-lg bg-green-50 dark:bg-green-900 shadow flex flex-col gap-4 mb-4">
      <h3 className="text-md font-bold text-green-900 dark:text-green-100 flex items-center gap-2">
        <span role="img" aria-label="Rack">🗄️</span> Racks
      </h3>
        ...
        <SortableContext items={items} strategy={verticalListSortingStrategy}>
          <div className="flex gap-4">
            {items.map((id: string) => {
              ...
              return rack ? <DraggableItem key={rack.id} id={rack.id} label={rack.name} type="rack" /> : null;
            })}
          </div>
        </SortableContext>
      </DndContext>
