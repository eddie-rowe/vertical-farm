...
import { FaSeedling, FaCog, FaUsers, FaTruck, FaExclamationCircle, FaClock } from 'react-icons/fa';
...
export function EventComponent({ event, timeText }: EventContentArg) {
  ...
  const eventType = (event.extendedProps?.eventType as EventType) || 'operations';
      ...
      {!isShortEvent && (
        <div className="flex items-center justify-between mt-0.5 text-xs opacity-80">
          ...
          <span className="text-xs opacity-75">
            {timeText || startTime.toLocaleTimeString([], { 
              hour: '2-digit', 
              minute: '2-digit',
              hour12: false
            })}
