...
const IntegrationCard: React.FC<IntegrationCardProps> = ({
  ...
  return (
    <div
      className={`relative p-4 rounded-lg border-2 transition-all duration-200 cursor-pointer ${getStatusColor()}`}
    ...
    >
      ...
      {/* Benefit description */}
      <p className="text-sm text-control-content mb-4 leading-relaxed">
        {benefit}
      </p>
      ...
      <div className="flex items-center justify-between">
        ...
        {status === "connected" && (
          <span className="text-sm text-farm-accent font-medium">
            Connected
          </span>
        )}

        {status === "coming-soon" && (
          <span className="text-sm text-control-label font-medium">
            Coming Soon
          </span>
        )}
      </div>
