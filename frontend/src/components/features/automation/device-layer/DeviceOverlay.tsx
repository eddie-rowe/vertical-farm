...
function DeviceStatusIndicator({ device, size = 'md', onClick }: DeviceStatusIndicatorProps) {
  const getDeviceIcon = () => {
    switch (device.device_type) {
      case 'light':
        return <LightBulbIcon className={cn("h-4 w-4", size === 'sm' && "h-3 w-3", size === 'lg' && "h-5 w-5")} />;
      case 'pump':
        return <CircleStackIcon className={cn("h-4 w-4", size === 'sm' && "h-3 w-3", size === 'lg' && "h-5 w-5")} />;
      case 'fan':
        return <ArrowPathIcon className={cn("h-4 w-4", size === 'sm' && "h-3 w-3", size === 'lg' && "h-5 w-5")} />;
      case 'sensor':
        return <WrenchScrewdriverIcon className={cn("h-4 w-4", size === 'sm' && "h-3 w-3", size === 'lg' && "h-5 w-5")} />;
      default:
        return <PowerIcon className={cn("h-4 w-4", size === 'sm' && "h-3 w-3", size === 'lg' && "h-5 w-5")} />;
    }
  };
  ...
  const getStatusColor = () => {
    ...
    switch (device.current_state) {
      case 'on':
        return device.device_type === 'light' ? 'bg-yellow-400' : 'bg-green-400';
      case 'off':
        return 'bg-gray-600';
      case 'unavailable':
        return 'bg-red-400';
...
function QuickControls({ device, onControl }: QuickControlsProps) {
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="text-sm font-medium">{device.device_name || device.device_type}</div>
      <div className="text-xs text-muted-foreground">
        Status: <Badge variant={device.is_online ? "default" : "destructive"}>
      ...
      {canControl && (
        <div className="flex gap-1">
          ...
          {device.device_type === 'light' && device.capabilities?.brightness && (
            <Button
              size="sm"
              variant="outline"
              onClick={() => handleControl({ type: 'set_brightness', brightness: 128 })}
              disabled={loading !== null}
              className="h-6 px-2 text-xs"
            >
              50%
    ...
    <TooltipProvider>
      ...
      >
        ...
        <div className="relative h-full w-full">
          ...
          {devices.some(d => !d.is_online || d.current_state === 'unavailable') && (
