...
interface UnifiedDeviceCardProps {
  ...
  onDeviceControl: (device: UnifiedDevice, action: 'turn_on' | 'turn_off' | 'toggle') => void;
      ...
      <CardContent className="p-4">
          ...
          <div className="flex items-start justify-between flex-1">
            <div className="flex items-start gap-3 flex-1">
              <div className="flex-shrink-0 mt-1">
                {getDeviceIcon(device.device_type || device.domain, device.domain)}
              </div>
              <div className="flex-1 min-w-0">
                <h3 className="font-semibold text-sm leading-tight truncate">
                  {device.friendly_name || device.device_name || device.entity_id}
                </h3>
        ...
        <div className="flex items-center justify-between mb-3 text-xs text-gray-500">
          <div className="flex items-center gap-2">
            ...
            <div className={`flex items-center gap-1 px-2 py-1 rounded-full border text-xs ${getIntegrationColor(device.integration_type)}`}>
        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-2">
            ...
            {(device.domain !== 'light' && device.domain !== 'switch' && device.domain !== 'sensor') && (
              <FarmControlButton 
                size="default" 
                variant="default" 
                className="flex-1"
