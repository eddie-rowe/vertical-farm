...
export function GrowDeviceControlPanel({
  scheduleId,
  shelfId,
  shelfName,
  ...
  const [lastActions, setLastActions] = useState<
    Record<
      string,
      { action: string; status: "success" | "error"; timestamp: Date }
    >
  >({});
  ...
  const getLastActionStatus = (deviceType: string) => {
    ...
    return (
      <div className="flex items-center gap-1 text-xs">
        ...
        >
          {lastAction.action}{" "}
          {lastAction.status === "success" ? "sent" : "failed"}
        </span>
      </div>
    );
  };
      ...
      <CardContent>
        ...
        <div className="mt-4 p-3 bg-blue-50 rounded-lg">
          <p className="text-sm text-blue-800">
            <strong>Real-time Control:</strong> Device commands are sent
            immediately to Home Assistant. Environmental conditions are
            monitored automatically and devices may be controlled automatically
            based on your grow recipe parameters.
          </p>
        </div>
      </CardContent>
    </Card>
