...
export default function AutomationMonitorCard({ 
  ...
  useEffect(() => {
    const mockStatus: AutomationStatus = {
      ...
      last_automation_check: new Date(Date.now() - 15 * 60 * 1000).toISOString(), // 15 minutes ago
      next_automation_action: new Date(Date.now() + 45 * 60 * 1000).toISOString(), // 45 minutes from now
      pending_tasks: 2,
      processing_tasks: 1,
  ...
  const getActionIcon = (actionType: string) => {
    switch (actionType) {
      case 'environmental_check': return <Thermometer className="h-3 w-3" />;
      case 'lighting_cycle': return <Sun className="h-3 w-3" />;
      case 'watering_cycle': return <Droplets className="h-3 w-3" />;
      case 'device_control': return <Zap className="h-3 w-3" />;
    ...
    <Card className={className}>
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <Badge variant={automationStatus.automation_enabled ? "default" : "secondary"}>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-3 gap-2 text-xs">
          <div className="text-center">
            <div className="font-medium text-yellow-600">{automationStatus.pending_tasks}</div>
            <div className="text-gray-500">Pending</div>
          </div>
          <div className="text-center">
            <div className="font-medium text-blue-600">{automationStatus.processing_tasks}</div>
            <div className="text-gray-500">Processing</div>
          </div>
          <div className="text-center">
            <div className="font-medium text-red-600">{automationStatus.failed_tasks_today}</div>
        ...
        <div className="space-y-2 text-xs">
          {automationStatus.last_automation_check && (
            <div className="flex items-center justify-between">
              ...
              <span>{formatTimeAgo(automationStatus.last_automation_check)}</span>
            </div>
          )}
          {automationStatus.next_automation_action && (
            <div className="flex items-center justify-between">
              <span className="text-gray-500">Next action:</span>
              <span>{formatTimeFromNow(automationStatus.next_automation_action)}</span>
            </div>
          )}
        </div>
        ...
        <div className="space-y-2">
          <div className="text-xs font-medium text-gray-700 dark:text-gray-300">Recent Actions</div>
          <div className="space-y-1">
            {automationStatus.recent_actions.slice(0, 3).map((action, index) => (
              <div key={index} className="flex items-center gap-2 text-xs">
                {getActionIcon(action.type)}
                <span className="flex-1 truncate">{action.type.replace('_', ' ')}</span>
        ...
        {automationStatus.environmental_alerts.length > 0 && (
          <div className="space-y-2">
            <div className="text-xs font-medium text-gray-700 dark:text-gray-300">Alerts</div>
            <div className="space-y-1">
              {automationStatus.environmental_alerts.slice(0, 2).map((alert, index) => (
                <div key={index} className="flex items-start gap-2 text-xs">
                  {getAlertIcon(alert.severity)}
                  <div className="flex-1">
                    <div className="text-gray-600 dark:text-gray-400">{alert.message}</div>
                    <div className="text-gray-500">{formatTimeAgo(alert.timestamp)}</div>
                  </div>
                </div>
              ))}
            </div>
