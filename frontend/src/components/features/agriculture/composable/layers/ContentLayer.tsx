      ...
      <div className="flex items-center justify-between">
        ...
        {content.showMetrics && (
          <div className="flex space-x-4 text-sm text-gray-600 dark:text-gray-400">
            ...
            <span>
              Total Shelves: {row.racks?.reduce((acc, rack) => acc + (rack.shelves?.length || 0), 0) || 0}
            </span>
          </div>
        )}
      </div>
      ...
      <div className="p-4 border-b border-gray-200 dark:border-gray-700">
        ...
        {content.showMetrics && (
          <div className="mt-2 text-xs text-gray-600 dark:text-gray-400">
            <span>{getOccupiedShelves()}/{getShelfCount()} shelves occupied</span>
          </div>
        )}
      </div>
...
export function ShelfContent({ shelf, areaType, shelfIndex, className }: ShelfContentProps) {
  const configuration = useAreaConfiguration()
  const { content } = configuration
    ...
    <div className="space-y-2">
      ...
      <div className="flex items-center justify-between text-xs">
        ...
        {mockGerminationData.readyForTransplant && (
          <Badge variant="outline" className="text-xs py-0 px-1 border-green-500 text-green-700 dark:text-green-400">
            Ready
          </Badge>
        )}
      </div>
    ...
    <div className="space-y-2">
      ...
      <div className="flex items-center justify-between text-xs">
        ...
        <Badge 
          ...
          className={cn(
            "text-xs py-0 px-1",
            mockPlantData.health === 'excellent' && "border-green-500 text-green-700 dark:text-green-400"
          )}
        >
...
export function getContentStyleClasses(style: 'minimal' | 'standard' | 'detailed') {
  switch (style) {
    case 'minimal':
