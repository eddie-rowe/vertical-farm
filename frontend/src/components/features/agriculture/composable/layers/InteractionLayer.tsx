...
import { useAreaInteractions, useAreaActions, useAreaState } from '../providers/FarmAreaProvider'
...
export function InteractionLayer({ children, className }: InteractionLayerProps) {
  return (
...
export function RowInteraction({ row, children, className }: RowInteractionProps) {
  const { isDoubleClickEnabled, isSelectionEnabled, actions } = useAreaInteractions()
...
export function RackInteraction({ rack, children, className }: RackInteractionProps) {
  const { isDoubleClickEnabled, isSelectionEnabled, actions } = useAreaInteractions()
...
export function ShelfInteraction({ shelf, children, className }: ShelfInteractionProps) {
  const { isDoubleClickEnabled, isSelectionEnabled, actions } = useAreaInteractions()
...
export function ContextMenu({ children, items, target }: ContextMenuProps) {
  ...
  const handleContextMenu = useCallback((e: React.MouseEvent) => {
    if (isContextMenuEnabled) {
      ...
      toast.success(`Context menu for ${target} (${items.length} actions available)`)
    }
...
export function useSelectionUtils() {
  const { selectedRow, selectedRack, selectedShelf, selectedElement } = useAreaState()
