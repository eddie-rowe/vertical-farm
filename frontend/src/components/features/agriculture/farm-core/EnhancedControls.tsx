...
const EnhancedControls: React.FC<EnhancedControlsProps> = ({
  ...
  const handleWheel = useCallback(
    (event: WheelEvent) => {
      ...
      const newPanX =
        viewState.panX - (mouseX - rect.width / 2) * (zoomFactor - 1);
      const newPanY =
        viewState.panY - (mouseY - rect.height / 2) * (zoomFactor - 1);
    ...
    (event: React.TouchEvent) => {
      ...
      if (
        ...
        if (lastTouchDistance.current > 0) {
          ...
          const newZoom = Math.max(
            MIN_ZOOM,
            Math.min(MAX_ZOOM, viewState.zoom * scale),
          );
          onViewStateChange({ ...viewState, zoom: newZoom });
        }
    ...
    <div className={`${className}`}>
      ...
      <div className="fixed bottom-4 left-4 z-30 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg px-3 py-2">
        <div className="flex items-center space-x-4 text-xs text-gray-600 dark:text-gray-400">
          ...
          <span>
            Pan: {Math.round(viewState.panX)}, {Math.round(viewState.panY)}
          </span>
