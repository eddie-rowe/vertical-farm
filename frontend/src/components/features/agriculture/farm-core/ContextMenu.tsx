...
export default function ContextMenu({ 
  ...
  const getMenuItems = (): Array<{ icon: any; label: string; action: string; destructive?: boolean } | { divider: true }> => {
    const { type } = element;
    ...
    const typeSpecificItems: Record<string, Array<{ icon: any; label: string; action: string; destructive?: boolean }>> = {
      farm: [
      ...
      row: [
        ...
        { icon: Trash2, label: 'Delete Row', action: 'delete', destructive: true },
      ],
      rack: [
        { icon: Plus, label: 'Add Shelf', action: 'add-shelf' },
        { icon: Trash2, label: 'Delete Rack', action: 'delete', destructive: true },
      ],
      shelf: [
        { icon: Plus, label: 'Add Device', action: 'add-device' },
        { icon: Trash2, label: 'Delete Shelf', action: 'delete', destructive: true },
      ],
      device: [
        { icon: Settings, label: 'Configure', action: 'configure' },
        { icon: Eye, label: 'Show Data', action: 'show-data' },
        { icon: Trash2, label: 'Remove Device', action: 'delete', destructive: true },
      ]
    };
