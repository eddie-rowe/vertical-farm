...
export function LayerSwitcher({
  ...
  const getLayerDescription = (layer: LayerType) => {
    switch (layer) {
      case "devices":
        return "View device assignments and status";
      case "automation":
        return "Automation rules and schedules";
      case "monitoring":
        return "Real-time environmental monitoring";
      case "grows":
        return "Current grow cycles and progress";
    }
  };
  ...
  const getButtonSize = () => {
    switch (size) {
      case "sm":
        return showLabels ? "sm" : "sm";
      case "md":
        return showLabels ? "default" : "sm";
      case "lg":
        return showLabels ? "lg" : "default";
    }
  };
            ...
            <div key={layer} className="relative">
              ...
              >
                <div className="flex items-center gap-1">
                  {getLayerIcon(layer)}
                  {showLabels && (
                    <span className="text-xs">{getLayerLabel(layer)}</span>
                  )}
                </div>
              </Button>
          ...
          >
            ...
            {showLabels && (
              <span className="text-xs font-medium">
                {activeLayer
                  ? `${getLayerLabel(activeLayer)} Active`
                  : "No Active Layer"}
              </span>
            )}
          </div>
