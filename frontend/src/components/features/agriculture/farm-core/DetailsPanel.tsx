...
export default function DetailsPanel({
  ...
  const renderActivityTab = () => (
    <div className="space-y-3">
      {currentActivity.map((item) => (
        <div
          key={item.id}
          className="flex items-start space-x-3 p-3 rounded-lg border border-gray-100 dark:border-gray-800"
        >
    ...
    if (!selectedElement.type || !selectedElement.data) {
      return (
        <div className="text-center py-8 text-gray-500 dark:text-gray-400">
          ...
          <p className="text-sm">
            Select a farm element to view detailed information
          </p>
        </div>
      );
    }
  ...
  return (
    <Card
      className={`transition-all duration-300 ${isExpanded ? "h-80" : "h-16"}`}
    >
