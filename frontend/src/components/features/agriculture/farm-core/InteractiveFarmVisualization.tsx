...
const InteractiveFarmVisualization: React.FC<InteractiveFarmVisualizationProps> = ({
  farmData,
  selectedRow,
  selectedRack,
  selectedShelf,
  ...
  const handleContextAction = useCallback(async (action: ContextAction) => {
    ...
    try {
      const { createRow, createRack, createShelf } = await import('@/services/supabaseService')
    ...
    return (
      <div
        ...
        className={cn(
          ...
          isSelected && "ring-2 ring-blue-500 border-blue-300 dark:border-blue-600 shadow-lg bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-900/30 dark:to-blue-800/30",
          ...
          <div className="mt-3 pt-3 border-t border-slate-200 dark:border-slate-600">
            ...
            <div className="h-2 bg-slate-200 dark:bg-slate-600 rounded-full overflow-hidden">
              <div 
                ...
                style={{ width: `${Math.min((shelves.length / 6) * 100, 100)}%` }}
              />
          </div>
          </div>
        )}
    ...
    return (
      <div
        ...
        className={cn(
          ...
          isSelected && "ring-2 ring-blue-500 border-blue-400 dark:border-blue-500 shadow-xl bg-gradient-to-br from-blue-50 via-blue-100 to-blue-150 dark:from-blue-900/40 dark:via-blue-800/40 dark:to-blue-700/40",
        ...
        {(isHovered || isSelected) && (
          <div className="mt-4 pt-4 border-t border-slate-300 dark:border-slate-500">
            <div className="grid grid-cols-3 gap-4 text-center">
              <div>
                <div className="text-2xl font-bold text-blue-600 dark:text-blue-400">{racks.length}</div>
                <div className="text-xs text-slate-600 dark:text-slate-300">Racks</div>
              </div>
              <div>
                <div className="text-2xl font-bold text-green-600 dark:text-green-400">{totalShelves}</div>
                <div className="text-xs text-slate-600 dark:text-slate-300">Shelves</div>
              </div>
              <div>
                <div className="text-2xl font-bold text-purple-600 dark:text-purple-400">{totalDevices}</div>
    ...
    return (
      <div className="flex items-center justify-center h-96 bg-slate-50 dark:bg-slate-800 rounded-2xl border-2 border-dashed border-slate-300 dark:border-slate-600">
        <div className="text-center">
          ...
          <h3 className="text-xl font-semibold text-slate-600 dark:text-slate-300 mb-2">No Farm Data</h3>
          <p className="text-slate-500 dark:text-slate-400">Unable to load farm visualization data.</p>
        </div>
      </div>
    )
  }
