...
const UnifiedFarmView: React.FC<UnifiedFarmViewProps> = ({
  ...
  // State management
  const { hoveredElements, setHoveredElement, clearHoveredElement } =
    useFarmState();
  ...
  const handleGrowSetup = useCallback(
    async (growData: any) => {
  ...
  const handleContextAction = useCallback(
    async (action: ContextAction, element?: Row | Rack | Shelf) => {
      ...
      try {
        switch (action) {
          case "add-row": {
            ...
            const nextPosition = await rowService.getNextPosition(
              farmData.farm.id,
            );

            // Generate unique name
            const existingRows = await rowService.getRowsByFarm(
              farmData.farm.id,
            );
          ...
          case "add-rack":
            if (selectedRow) {
              ...
              const nextPosition = await rackService.getNextPosition(
                selectedRow.id,
              );

              // Generate unique name
              const existingRacks = await rackService.getRacksByRow(
                selectedRow.id,
              );
          ...
          case "add-shelf":
            if (selectedRack) {
              ...
              const nextPosition = await shelfService.getNextPosition(
                selectedRack.id,
              );

              // Generate unique name
              const existingShelves = await shelfService.getShelvesByRack(
                selectedRack.id,
              );
  ...
  const renderShelf = (shelf: Shelf) => {
    ...
    console.log(
      `Rendering shelf: ${shelf.name}, ID: ${shelf.id}, devices:`,
      devices,
    );
    ...
    const generateMockDeviceStatus = (elementId: string) => {
      const seed = parseInt(elementId) || 1;
      return {
        connectivity: (["connected", "disconnected", "poor"] as const)[
          seed % 3
        ],
        ...
        {racks.length > 0 ? (
          ...
          <div className="text-center py-16 text-green-500 dark:text-green-400">
            ...
            <p className="text-base opacity-75 mb-6">
              Add racks to organize your growing space
            </p>
            <Button
              onClick={(e) => {
                e.stopPropagation();
    ...
    <div className="relative w-full h-full bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden">
      ...
      <div className="p-8 h-full overflow-auto">
        {rows.length === 0 ? (
          ...
          <div className="space-y-8 pr-20">{rows.map(renderRow)}</div>
        )}
      </div>
