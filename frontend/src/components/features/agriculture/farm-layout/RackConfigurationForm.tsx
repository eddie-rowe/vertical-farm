...
export default function RackConfigurationForm({
  rack,
  onShelvesReorder,
}: {
  rack: Rack;
  onShelvesReorder?: (newShelves: Shelf[]) => void;
}) {
  ...
  const [items, setItems] = React.useState<string[]>(
    rack.shelves.map((shelf) => shelf.id),
  );
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    if (over && String(active.id) !== String(over.id)) {
      ...
      setItems(newItems);
      if (onShelvesReorder) {
        const newShelves = newItems.map(
          (id) => rack.shelves.find((shelf) => shelf.id === id) as Shelf,
        );
