...
export const generateMockGrows = (count: number = 50): GrowTimelineItem[] => {
  ...
  const recipes = ['Lettuce Mix', 'Herb Garden', 'Microgreens', 'Spinach Blend', 'Kale Variety'];
  const species = ['Lettuce', 'Basil', 'Cilantro', 'Spinach', 'Kale', 'Arugula'];
  const statuses: Array<'planned' | 'active' | 'completed' | 'aborted'> = ['planned', 'active', 'completed', 'aborted'];
  ...
  return Array.from({ length: count }, (_, i) => {
    ...
    startDate.setDate(startDate.getDate() - Math.floor(Math.random() * 120) - 30); // Started 30-150 days ago
    ...
    return {
      id: `grow-${i + 1}`,
      shelfId: `${farmName}-${rowName}-${rackName}-${shelfName}`.replace(/\s/g, '-').toLowerCase(),
      shelfName,
      rackName,
      rowName,
      farmName,
      ...
      daysRemaining,
      totalDays,
      yield: status === 'completed' ? Math.floor(Math.random() * 500) + 100 : undefined,
...
export const filterGrows = (
  ...
  return grows.filter(grow => {
    ...
    const matchesStatus = !statusFilter || statusFilter === 'all' || grow.status === statusFilter;
