...
import { TimelineState, GrowTimelineItem, ViewMode, SortField, SortOrder } from '../types';
...
export const useGrowTimeline = ({
  ...
  const processedGrows = useMemo(() => {
    const filtered = filterGrows(state.grows, state.searchTerm, state.statusFilter);
    ...
    return filtered;
  }, [state.grows, state.searchTerm, state.statusFilter, state.sortBy, state.sortOrder]);
  ...
  const selectGrow = useCallback((growId: string, multi = false) => {
    setState(prev => {
  ...
  const handleGrowAction = useCallback((growId: string, action: string) => {
    if (onGrowAction) {
    ...
    switch (action) {
      ...
      case 'complete':
        setState(prev => ({
          ...prev,
          grows: prev.grows.map(grow =>
            grow.id === growId ? { ...grow, status: 'completed' as const, progress: 100 } : grow
          ),
        }));
        break;
    }
  ...
  return {
    ...
    // Computed
    selectedGrow: state.selectedGrows.length === 1 
