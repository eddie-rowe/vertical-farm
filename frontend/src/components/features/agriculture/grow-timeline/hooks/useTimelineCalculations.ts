...
export const useTimelineCalculations = ({
  ...
  const timelineData = useMemo(() => {
    if (grows.length === 0) return { positions: new Map(), timelineStart: currentDate, timelineEnd: currentDate };
  ...
  const generateTimeMarkers = useMemo(() => {
    ...
    if (zoomLevel >= 4) intervalDays = 1; // Daily for high zoom
    else if (zoomLevel >= 2) intervalDays = 3; // Every 3 days for medium zoom
