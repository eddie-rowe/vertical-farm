...
const FarmAreaGrid: React.FC<FarmAreaGridProps> = ({
  ...
  const handleDoubleClick = useCallback((element: Row | Rack | Shelf, elementType: 'row' | 'rack' | 'shelf') => {
    ...
    toast.success(`Opening ${elementType} details: ${element.name || element.id}`)
  }, [])
  ...
  const renderGerminationShelfContent = (shelf: Shelf) => {
    ...
    const daysSincePlanting = germinationData.planting_date 
      ? Math.floor((Date.now() - new Date(germinationData.planting_date).getTime()) / (1000 * 60 * 60 * 24))
      : 0
    ...
    return (
      <div className="space-y-1">
        ...
        {germinationData.is_transplant_ready && (
          <Badge variant="default" className="text-xs bg-green-100 text-green-800 dark:bg-green-800 dark:text-green-200">
            Ready
          </Badge>
        )}
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <div
              className={cn(
                ...
                isSelected && "ring-2 ring-blue-500 border-blue-300 dark:border-blue-600 bg-gradient-to-r from-blue-50 to-blue-100 dark:from-blue-900/30 dark:to-blue-800/30",
              ...
              onMouseEnter={() => setHoveredElements(prev => ({ ...prev, shelf: shelf.id }))}
              onMouseLeave={() => setHoveredElements(prev => ({ ...prev, shelf: undefined }))}
            >
              {/* Shelf header */}
          ...
          <TooltipContent>
            <div className="space-y-1">
              ...
              <div className="text-sm">Devices: {shelf.devices?.length || 0}</div>
              {areaType === 'germination_tent' && shelf.germination_data && (
                <div className="text-sm border-t pt-1">
                  ...
                  {shelf.germination_data.planting_date && (
                    <div>Planted: {new Date(shelf.germination_data.planting_date).toLocaleDateString()}</div>
                  )}
                </div>
              )}
            </div>
    ...
    return (
      <div
        ...
        className={cn(
          ...
          isSelected && "ring-2 ring-blue-500 border-blue-400 dark:border-blue-500 bg-gradient-to-r from-blue-100 to-blue-200 dark:from-blue-800/40 dark:to-blue-700/40",
        ...
        onMouseEnter={() => setHoveredElements(prev => ({ ...prev, rack: rack.id }))}
        onMouseLeave={() => setHoveredElements(prev => ({ ...prev, rack: undefined }))}
      >
        {/* Rack Header */}
    ...
    return (
      <div
        ...
        className={cn(
          ...
          isSelected && "ring-2 ring-green-500 border-green-400 dark:border-green-500 bg-gradient-to-r from-green-100 to-emerald-150 dark:from-green-800/40 dark:to-emerald-800/40",
        ...
        onMouseEnter={() => setHoveredElements(prev => ({ ...prev, row: row.id }))}
        onMouseLeave={() => setHoveredElements(prev => ({ ...prev, row: undefined }))}
      >
        {/* Row Header */}
  ...
  if (filteredRows.length === 0) {
    return (
      <div className="text-center py-12">
        <IconComponent className={cn("h-12 w-12 mx-auto mb-4", config.color, "opacity-50")} />
        ...
        {onAddRow && (
          <Button onClick={onAddRow}>
            ...
            Add {areaType === 'germination_tent' ? 'Germination Tent' : 'Growing Area'}
          </Button>
        )}
      </div>
    )
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        {onAddRow && (
          <Button onClick={onAddRow} variant="outline" className="dark:border-gray-600 dark:text-gray-300">
            <Plus className="h-4 w-4 mr-2" />
