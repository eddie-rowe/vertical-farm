...
export default function HeatmapsView() {
  ...
  const getHeatmapColor = (value: number, metric: string) => {
    ...
    switch (metric) {
      case "temperature":
        intensity =
          value > 25 ? 0.8 : value > 20 ? 0.6 : value > 15 ? 0.4 : 0.2;
        return `rgba(255, 87, 34, ${intensity})`;
      case "humidity":
        intensity =
          value > 70 ? 0.8 : value > 60 ? 0.6 : value > 50 ? 0.4 : 0.2;
      ...
      default:
        intensity =
          value > 90 ? 0.8 : value > 75 ? 0.6 : value > 60 ? 0.4 : 0.2;
        return `rgba(156, 39, 176, ${intensity})`;
    }
  };

  const getCurrentOption = () =>
    heatmapOptions.find((opt) => opt.id === selectedHeatmap);
        ...
        <div className="space-y-4">
          <div>
            ...
            <div className="flex gap-2 flex-wrap">
              {heatmapOptions
                .filter((opt) => opt.category === "operations")
                .map((option) => (
                  <Button
                    key={option.id}
                    variant={
                      selectedHeatmap === option.id ? "default" : "outline"
                    }
                    size="sm"
          ...
          <div>
            ...
            <div className="flex gap-2 flex-wrap">
              {heatmapOptions
                .filter((opt) => opt.category === "environmental")
                .map((option) => (
                  <Button
                    key={option.id}
                    variant={
        ...
        <div className="lg:col-span-2">
          <Card className="p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <Badge variant="outline">{selectedTimeframe} view</Badge>
            </div>
              ...
              <svg width="100%" height="400" viewBox="0 0 600 400">
                ...
                {currentHeatmapData.map((point) => (
                  <g key={point.id}>
                    <circle
                      ...
                      onClick={() =>
                        setSelectedZone(
                          mockZones.find((z) => z.id === point.id) || null,
                        )
                      }
                    />
                    <text
                ...
                <g transform="translate(450, 300)">
                  ...
                  {[0.2, 0.4, 0.6, 0.8].map((intensity, index) => (
                    <g
                      key={index}
                      transform={`translate(0, ${(index + 1) * 20})`}
                    >
                      <circle
                        cx="10"
                        cy="0"
            ...
            <div className="mt-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4">
              ...
              <div className="space-y-2 text-sm">
                <p className="text-blue-800 dark:text-blue-200">
                  • Growth Tower 1 shows 15% higher {selectedHeatmap} compared
                  to average
                </p>
                <p className="text-blue-800 dark:text-blue-200">
                  • Greenhouse B could benefit from optimization to match Tower
                  performance
                </p>
                <p className="text-blue-800 dark:text-blue-200">
                  • {getCurrentOption()?.label} patterns suggest potential for
            ...
            {selectedZone ? (
              <div className="space-y-4">
                <div>
                  ...
                  <Badge
                    className={`mt-1 ${
                      selectedZone.type === "greenhouse"
                        ? "bg-green-100 text-green-800"
                        : selectedZone.type === "growth_tower"
                          ? "bg-blue-100 text-blue-800"
                          : selectedZone.type === "processing"
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {Object.entries(selectedZone.metrics).map(
                      ([key, value]) => (
                        <div
                          key={key}
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-medium">
                        {selectedZone.environmental.temperature}°C
                      </span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span>Humidity</span>
                      <span className="font-medium">
                        {selectedZone.environmental.humidity}%
                      </span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span>CO₂ Level</span>
                      <span className="font-medium">
                        {selectedZone.environmental.co2} ppm
                      </span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span>Light Level</span>
                      <span className="font-medium">
                        {selectedZone.environmental.lightLevel} μmol
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      <span>Revenue</span>
                      <span className="font-medium">
                        ${selectedZone.business.revenue.toLocaleString()}
                      </span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span>Costs</span>
                      <span className="font-medium">
                        ${selectedZone.business.costs.toLocaleString()}
                      </span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span>Profit</span>
                      <span className="font-medium text-green-600">
                        ${selectedZone.business.profit.toLocaleString()}
                      </span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span>ROI</span>
                      <span className="font-medium">
                        {selectedZone.business.roi}%
                      </span>
                    </div>
                  </div>
                </div>
              ...
              <div className="text-center text-gray-500 py-8">
                ...
                <p>
                  Click on a zone in the heatmap to view detailed information
                </p>
              </div>
            )}
          </Card>
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                ...
                <span className="font-bold">
                  {(
                    currentHeatmapData.reduce(
                      (sum, point) => sum + point.value,
                      0,
                    ) / currentHeatmapData.length
                  ).toFixed(1)}
                  {selectedHeatmap === "revenue"
                    ? "K"
                    : selectedHeatmap === "temperature"
                      ? "°C"
                      : selectedHeatmap.includes("roi") ||
                          selectedHeatmap.includes("humidity")
                        ? "%"
                        : ""}
                </span>
              </div>
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="font-bold text-blue-600">
                  {(
                    ...
                    Math.min(...currentHeatmapData.map((p) => p.value))
                  ).toFixed(1)}
                </span>
              </div>
            </div>
