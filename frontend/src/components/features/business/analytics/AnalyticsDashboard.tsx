...
const generateInsights = (): InsightAlert[] => [
  {
    ...
    description: 'Zone A is performing 12% above target yield due to consistent environmental conditions.',
    priority: 'low',
  ...
  },
  {
    ...
    description: 'Consider reducing light intensity in Zone D during peak hours to save energy while maintaining growth.',
    priority: 'medium',
  ...
  },
  {
    ...
    description: 'Zone B temperature variance is 15% higher than optimal. Check HVAC system.',
    priority: 'high',
    action: 'Check HVAC',
...
export const AnalyticsDashboard: React.FC<AnalyticsDashboardProps> = ({
  ...
  const [visibleWidgets, setVisibleWidgets] = useState<string[]>(['environmental-trends', 'yield-comparison', 'alert-distribution', 'growth-rate']);

  const kpiMetrics = useMemo(() => generateKPIMetrics(), [selectedTimeRange]);
  const chartWidgets = useMemo(() => generateChartWidgets(), [selectedTimeRange]);
  ...
  const getTrendDisplay = (trend: string, trendValue: string) => {
    ...
    const color = isPositive ? 'text-emerald-600' : trend === 'down' ? 'text-red-600' : 'text-gray-600';
  ...
  const getInsightColor = (priority: string) => {
    switch (priority) {
      ...
      case 'medium': return 'border-amber-200 bg-amber-50 dark:bg-amber-900/10';
      case 'low': return 'border-emerald-200 bg-emerald-50 dark:bg-emerald-900/10';
        ...
        {kpiMetrics.map((metric) => (
          ...
          >
            <div className="flex items-center justify-between mb-2">
              <div className={`p-2 rounded-lg ${getKPIStatusColor(metric.status)}`}>
                {metric.icon}
              </div>
              ...
              <div className="p-4">
                <DataChart
                  ...
                  config={{
                    ...
                    colors: widget.config.colors || ['#10b981', '#3b82f6', '#f59e0b'],
                    formatters: widget.type === 'pie' ? {} : {
                      temperature: (value: number) => `${value.toFixed(1)}°C`,
                      humidity: (value: number) => `${value.toFixed(0)}%`,
                      light: (value: number) => `${(value/1000).toFixed(0)}k lux`,
