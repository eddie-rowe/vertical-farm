...
interface GrowthTrackingChartProps {
  ...
  selectedMetrics?: Array<'height' | 'leafCount' | 'biomass' | 'leafArea' | 'rootLength' | 'chlorophyllContent'>;
}
...
const DEFAULT_GROWTH_METRICS: Array<keyof typeof GROWTH_METRIC_CONFIGS> = ['height', 'leafCount'];
...
export const GrowthTrackingChart: React.FC<GrowthTrackingChartProps> = ({
  ...
  const chartConfig: ChartConfig = {
    ...
    colors: selectedMetrics.map(metric => GROWTH_METRIC_CONFIGS[metric].color),
    formatters: {
      ...Object.fromEntries(
  ...
  const getGrowthRate = (metric: keyof typeof GROWTH_METRIC_CONFIGS) => {
    ...
    const rate = ((latest[metric] || 0) - (previous[metric] || 0)) / (latest.daysSinceSeeding - previous.daysSinceSeeding);
  ...
  const getCurrentGrowthStage = () => {
    ...
    if (currentDay <= 7) return { stage: 'Germination', color: 'text-yellow-600' };
    if (currentDay <= 14) return { stage: 'Seedling', color: 'text-green-500' };
    if (currentDay <= 21) return { stage: 'Vegetative', color: 'text-green-600' };
    if (currentDay <= 28) return { stage: 'Pre-harvest', color: 'text-blue-600' };
                  ...
                  <div key={metric} className="bg-white dark:bg-gray-800 p-4 rounded-lg border border-gray-200 dark:border-gray-700">
                    ...
                    {growthRate !== null && (
                      <div className={`text-sm ${growthRate >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                        {growthRate >= 0 ? '+' : ''}{growthRate.toFixed(2)} {config.unit}/day
                      </div>
                    )}
                  </div>
                );
