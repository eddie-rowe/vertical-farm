...
const generateSmartInsights = (): SmartInsight[] => [
  {
    ...
    description:
      "AI analysis shows 15% energy savings possible by adjusting LED schedules in Zone A during low-growth periods.",
    impact: "high",
    confidence: 92,
    category: "energy",
    data: {
  ...
  },
  {
    ...
    description:
      "Machine learning models predict significant yield improvement in upcoming harvest cycle based on current environmental conditions.",
    impact: "high",
    confidence: 87,
    category: "yield",
    data: {
  ...
  },
  {
    ...
    description:
      "Zone B showing atypical humidity fluctuations. Pattern analysis suggests potential HVAC system inefficiency.",
    impact: "medium",
    confidence: 94,
  ...
  },
  {
    ...
    description:
      "Analysis suggests adjusting nutrient concentration by 8% during early growth phases to improve overall plant health.",
    impact: "medium",
    confidence: 79,
    category: "yield",
    data: {
  ...
  },
  {
    ...
    description:
      "Water pump system showing early signs of wear. Recommend proactive maintenance within 2 weeks.",
    impact: "high",
    confidence: 85,
    ...
    predictions: [
      {
        ...
        value: 156,
        confidence: 92,
        factors: ["Temperature", "Light intensity", "Nutrient levels"],
      },
      {
        timeframe: "2 weeks",
        value: 312,
        confidence: 87,
        factors: ["Environmental stability", "Growth rate"],
      },
      {
        timeframe: "1 month",
        value: 645,
        confidence: 81,
        factors: ["Seasonal factors", "System efficiency"],
      },
    ],
  },
  {
    ...
    predictions: [
      {
        ...
        value: 1250,
        confidence: 96,
        factors: ["Lighting schedule", "HVAC demand"],
      },
      {
        timeframe: "7 days",
        value: 8750,
        confidence: 92,
        factors: ["Weather patterns", "Crop stage"],
      },
      {
        timeframe: "30 days",
        value: 37500,
        confidence: 85,
        factors: ["Seasonal demand", "System efficiency"],
      },
    ],
  },
];
...
export const SmartInsights: React.FC<SmartInsightsProps> = ({
  ...
  const getInsightIcon = (type: string, category: string) => {
    ...
    switch (category) {
      ...
      case "environmental":
        return <FaThermometerHalf className="h-5 w-5" />;
      case "efficiency":
        return <FaTint className="h-5 w-5" />;
      default:
  ...
  const giveFeedback = (insightId: string, positive: boolean) => {
    ...
    console.log(
      `Feedback for ${insightId}: ${positive ? "positive" : "negative"}`,
    );
  };
    ...
    <div className={`space-y-6 ${className}`}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="p-2 bg-emerald-100 dark:bg-emerald-900/30 rounded-lg">
            <FaBrain className="h-6 w-6 text-emerald-600 dark:text-emerald-400" />
          </div>
          <div>
          ...
          {models.map((model) => (
            ...
            >
              <div className="flex items-center justify-between mb-3">
                <h3 className="font-semibold text-content">{model.name}</h3>
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="flex flex-wrap gap-2">
          <span className="text-sm font-medium text-content-secondary self-center">
            Category:
          </span>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span
                        className={`px-2 py-1 text-xs font-medium rounded-full ${getStatusColor(insight.status)}`}
                      >
                ...
                <div className="mb-4 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 text-sm">
                    {insight.data.metrics && (
                      <div>
                        <h4 className="font-medium text-content-secondary mb-1">
                          Key Metrics
                        </h4>
                        {Object.entries(insight.data.metrics).map(
                          ([key, value]) => (
                            <div key={key} className="text-content-subtle">
                              {key}: {value}
                            </div>
                          ),
                        )}
                  ...
                  <div className="space-y-2">
                    {insight.actionItems.map((action) => (
                      ...
                      >
                        ...
                        <button className="ml-4 px-3 py-1 text-sm bg-emerald-600 dark:bg-emerald-700 text-white rounded-md hover:bg-emerald-700 dark:hover:bg-emerald-600 transition-colors">
                          Start
                        </button>
                      </div>
                    ))}
              ...
              <div className="flex justify-between items-center pt-4 border-t border-gray-200 dark:border-gray-700">
                <div className="flex gap-2">
                  ...
                  {insight.status === "acknowledged" && (
                    <button
                      onClick={() =>
                        updateInsightStatus(insight.id, "in-progress")
                      }
