...
export const PerformanceMetrics: React.FC<PerformanceMetricsProps> = ({
  ...
  const metrics = useMemo(() => generatePerformanceMetrics(selectedTimeframe), [selectedTimeframe]);
  ...
  const categories = [
    ...
    { id: 'yield', name: 'Yield', count: metrics.filter(m => m.category === 'yield').length },
    { id: 'efficiency', name: 'Efficiency', count: metrics.filter(m => m.category === 'efficiency').length },
    { id: 'environmental', name: 'Environmental', count: metrics.filter(m => m.category === 'environmental').length },
    { id: 'energy', name: 'Energy', count: metrics.filter(m => m.category === 'energy').length }
  ];

  const getStatusColor = (status: string) => {
    switch (status) {
      case 'excellent': return 'bg-emerald-50 border-emerald-200 text-emerald-800 dark:bg-emerald-900/20 dark:border-emerald-800 dark:text-emerald-400';
      case 'good': return 'bg-blue-50 border-blue-200 text-blue-800 dark:bg-blue-900/20 dark:border-blue-800 dark:text-blue-400';
      case 'warning': return 'bg-amber-50 border-amber-200 text-amber-800 dark:bg-amber-900/20 dark:border-amber-800 dark:text-amber-400';
  ...
  const getGoalStatusColor = (status: string) => {
    switch (status) {
      case 'on-track': return 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-400';
  ...
  const getTrendIcon = (trend: string) => {
    switch (trend) {
      case 'up': return <FaArrowUp className="h-4 w-4 text-emerald-600" />;
      case 'down': return <FaArrowDown className="h-4 w-4 text-red-600" />;
      case 'stable': return <FaMinus className="h-4 w-4 text-gray-600" />;
      default: return null;
    }
  };
