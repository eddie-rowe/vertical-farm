...
interface ResourceUsageChartProps {
  ...
  selectedResources?: Array<
    "waterUsage" | "electricityUsage" | "nutrientUsage" | "co2Usage"
  >;
}
...
const DEFAULT_RESOURCES: Array<keyof typeof RESOURCE_CONFIGS> = [
  "waterUsage",
  "electricityUsage",
  "nutrientUsage",
];
...
export const ResourceUsageChart: React.FC<ResourceUsageChartProps> = ({
  ...
  const chartConfig: ChartConfig = {
    ...
    colors: selectedResources.map(
      (resource) => RESOURCE_CONFIGS[resource].color,
    ),
  ...
  const getEfficiencyMetrics = () => {
    ...
    return {
      dailyWaterUsage:
        (totals.find((t) => t.resource === "waterUsage")?.total || 0) /
        Math.max(timeSpanDays, 1),
      dailyEnergyUsage:
        (totals.find((t) => t.resource === "electricityUsage")?.total || 0) /
        Math.max(timeSpanDays, 1),
