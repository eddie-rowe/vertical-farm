...
import { FaSeedling, FaChartBar, FaTable, FaBell, FaCog, FaQuestionCircle } from "react-icons/fa";
        ...
        <nav className="flex flex-col gap-2">
          {navItems.map(({ href, label, icon }) => (
            <Link
              key={href}
              href={href}
              className={`flex items-center gap-3 px-4 py-2 rounded-lg font-medium transition-all btn-animated text-lg ${
                pathname === href
