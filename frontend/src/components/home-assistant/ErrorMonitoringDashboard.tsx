...
const ErrorMonitoringDashboard: React.FC = () => {
  const [healthData, setHealthData] = useState<ErrorMonitoringData | null>(null)
  const [circuitBreakerData, setCircuitBreakerData] = useState<Record<string, CircuitBreakerInfo> | null>(null)
  ...
  const fetchHealthData = async () => {
    try {
      const response = await fetch('/api/v1/home-assistant/error-monitoring/health')
  ...
  const fetchCircuitBreakerData = async () => {
    try {
      const response = await fetch('/api/v1/home-assistant/error-monitoring/circuit-breaker-status')
  ...
  const fetchErrorPatterns = async () => {
    try {
      const response = await fetch('/api/v1/home-assistant/error-monitoring/error-patterns?hours=24')
  ...
  const handleTestRecovery = async () => {
    ...
    try {
      const response = await fetch('/api/v1/home-assistant/error-monitoring/test-recovery?service_name=user_home_assistant', {
        method: 'POST'
      })
    ...
    <div className="space-y-6">
      ...
      {healthData && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <Card>
            ...
            <CardContent>
              <div className="flex items-center space-x-2">
                {getStatusIcon(healthData.overall_healthy ? 'healthy' : 'unhealthy')}
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{healthData.total_errors}</div>
              <p className="text-xs text-muted-foreground">
                Across all services
              </p>
            </CardContent>
      ...
      <Tabs defaultValue="overview" className="space-y-4">
        ...
        <TabsContent value="overview">
          {errorPatterns && (
            <div className="space-y-4">
              {/* System Health Alert */}
              <Alert className={
                errorPatterns.overall_system_health === 'excellent' ? 'border-green-200 bg-green-50' :
                errorPatterns.overall_system_health === 'good' ? 'border-blue-200 bg-blue-50' :
                'border-yellow-200 bg-yellow-50'
              }>
                <TrendingUp className="h-4 w-4" />
                <AlertTitle>System Health: {errorPatterns.overall_system_health.toUpperCase()}</AlertTitle>
                <AlertDescription>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="space-y-3">
                      {Object.entries(errorPatterns.error_distribution).map(([errorType, count]) => {
                        const percentage = (count / errorPatterns.total_errors) * 100
                        return (
        ...
        <TabsContent value="services">
          {healthData && (
            <div className="grid gap-4">
              {Object.entries(healthData.services).map(([serviceName, health]) => (
                <Card key={serviceName}>
                  <CardHeader>
                    <div className="flex items-center justify-between">
                      <CardTitle className="flex items-center space-x-2">
                        {getStatusIcon(health.status)}
                        <span>{serviceName.replace('_', ' ').toUpperCase()}</span>
                      </CardTitle>
                  ...
                  <CardContent>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      <div>
                        ...
                        <Progress value={health.health_score} className="h-2 mt-1" />
                      </div>
                      <div>
                        <div className="text-sm font-medium">Error Rate</div>
                        <div className="text-2xl font-bold">{health.error_rate_percent.toFixed(2)}%</div>
                      </div>
                      <div>
                        <div className="text-sm font-medium">Total Errors</div>
                        <div className="text-2xl font-bold">{health.total_errors}</div>
                      </div>
                      <div>
                        <div className="text-sm font-medium">Recoveries</div>
                        <div className="text-2xl font-bold">{health.recovery_count}</div>
                      </div>
                    </div>

                    {Object.keys(health.error_types).length > 0 && (
                      <div className="mt-4">
                        <div className="text-sm font-medium mb-2">Error Types</div>
                        <div className="flex flex-wrap gap-2">
                          {Object.entries(health.error_types).map(([type, count]) => (
        ...
        <TabsContent value="circuit-breakers">
          {circuitBreakerData && (
            <div className="grid gap-4">
              {Object.entries(circuitBreakerData).map(([serviceName, breaker]) => (
                <Card key={serviceName}>
                  <CardHeader>
                    <div className="flex items-center justify-between">
                      <CardTitle className="flex items-center space-x-2">
                      ...
                      <div className="flex space-x-2">
                        ...
                        >
                          {breaker.circuit_state.replace('_', ' ').toUpperCase()}
                        </Badge>
                        <Badge variant={breaker.status === 'healthy' ? 'default' : 'destructive'}>
                          {breaker.status}
                        </Badge>
                      </div>
                    </div>
                  ...
                  <CardContent>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      <div>
                        ...
                        <div className="text-2xl font-bold">{breaker.failure_count}</div>
                      </div>
                      <div>
                        <div className="text-sm font-medium">CB Triggers</div>
                        <div className="text-2xl font-bold">{breaker.circuit_breaker_triggers}</div>
                      </div>
                      <div>
            ...
            <div className="space-y-6">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Analysis based on the last {errorPatterns.analysis_period_hours} hours
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="space-y-4">
                    {Object.entries(errorPatterns.service_reliability).map(([serviceName, stats]) => (
                      <div key={serviceName} className="border rounded-lg p-4">
                        <div className="flex items-center justify-between mb-3">
                          <h4 className="font-medium">{serviceName.replace('_', ' ').toUpperCase()}</h4>
                          <Badge className={getReliabilityColor(stats.reliability_tier)}>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                          <div>
                            ...
                            <div className="font-bold">{stats.health_score}%</div>
                          </div>
                          <div>
                            <span className="text-muted-foreground">Error Rate:</span>
                            <div className="font-bold">{stats.error_rate.toFixed(2)}%</div>
                          </div>
                          <div>
                            <span className="text-muted-foreground">Total Errors:</span>
                            <div className="font-bold">{stats.total_errors}</div>
                          </div>
                          <div>
                            <span className="text-muted-foreground">Recoveries:</span>
                            <div className="font-bold">{stats.recovery_count}</div>
                          </div>
                        </div>
                      </div>
                    ))}
