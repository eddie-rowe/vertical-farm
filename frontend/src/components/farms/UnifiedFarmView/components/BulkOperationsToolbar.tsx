...
import { Trash2, Copy, Move, CheckSquare, X, MoreHorizontal, Edit3, Archive } from 'lucide-react'
...
export const BulkOperationsToolbar: React.FC<BulkOperationsToolbarProps> = ({
  ...
  const handleBulkDelete = useCallback(() => {
    ...
    if (confirm(confirmMessage)) {
      try {
        ...
        toast.success(`Deleted ${totalSelected} ${totalSelected === 1 ? 'item' : 'items'}`)
      } catch (error) {
  ...
  const handleBulkCopy = useCallback(() => {
    ...
    try {
      ...
      toast.success(`Copied ${totalSelected} ${totalSelected === 1 ? 'item' : 'items'}`)
    } catch (error) {
        ...
        <div className="flex items-center gap-4">
            ...
            <div className="flex gap-1">
              {selectionSummary.row > 0 && (
                <Badge variant="secondary" className="text-xs">
                  {selectionSummary.row} row{selectionSummary.row !== 1 ? 's' : ''}
                </Badge>
              )}
              {selectionSummary.rack > 0 && (
                <Badge variant="secondary" className="text-xs">
                  {selectionSummary.rack} rack{selectionSummary.rack !== 1 ? 's' : ''}
                </Badge>
              )}
              {selectionSummary.shelf > 0 && (
                <Badge variant="secondary" className="text-xs">
                  {selectionSummary.shelf} shelf{selectionSummary.shelf !== 1 ? 'ves' : ''}
                </Badge>
              )}
            </div>
          </div>
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem onClick={() => toast('Archive feature coming soon')}>
              ...
              <DropdownMenuSeparator />
              <DropdownMenuItem onClick={() => toast('Properties feature coming soon')}>
