...
export const LayoutTemplateManager: React.FC<LayoutTemplateManagerProps> = ({
  ...
  const calculateStats = useCallback((layout: { rows: Row[] }) => {
    ...
    const totalRacks = layout.rows.reduce((acc, row) => acc + (row.racks?.length || 0), 0)
    const totalShelves = layout.rows.reduce((acc, row) => 
      acc + (row.racks?.reduce((rackAcc, rack) => 
        rackAcc + (rack.shelves?.length || 0), 0) || 0), 0)
    const totalDevices = layout.rows.reduce((acc, row) => 
      acc + (row.racks?.reduce((rackAcc, rack) => 
        rackAcc + (rack.shelves?.reduce((shelfAcc, shelf) => 
          shelfAcc + (shelf.devices?.length || 0), 0) || 0), 0) || 0), 0)
  ...
  const saveCurrentLayout = useCallback(() => {
    ...
    const tags = newTemplateTags.split(',').map(tag => tag.trim()).filter(Boolean)
  ...
  }, [currentLayout, newTemplateName, newTemplateDescription, newTemplateTags, calculateStats])
  ...
  const toggleFavorite = useCallback((templateId: string) => {
    setTemplates(prev => prev.map(template => 
      template.id === templateId 
        ? { ...template, metadata: { ...template.metadata, favorite: !template.metadata.favorite } }
        : template
    ))
  }, [])
  ...
  const applyTemplate = useCallback((template: LayoutTemplate) => {
    if (confirm(`Apply template "${template.name}"? This will replace the current layout.`)) {
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="text-center text-slate-500">
              ...
              <p className="text-sm">Save your current layout to create your first template.</p>
            </div>
          </CardContent>
        </Card>
      ) : (
              ...
              <CardContent className="pt-0">
                <div className="grid grid-cols-4 gap-4 mb-3">
                  <div className="text-center">
                    <div className="text-lg font-bold text-blue-600">{template.stats.totalRows}</div>
                    <div className="text-xs text-slate-500">Rows</div>
                  </div>
                  <div className="text-center">
                    <div className="text-lg font-bold text-green-600">{template.stats.totalRacks}</div>
                    <div className="text-xs text-slate-500">Racks</div>
                  </div>
                  <div className="text-center">
                    <div className="text-lg font-bold text-purple-600">{template.stats.totalShelves}</div>
                    <div className="text-xs text-slate-500">Shelves</div>
                  </div>
                  <div className="text-center">
                    <div className="text-lg font-bold text-orange-600">{template.stats.totalDevices}</div>
