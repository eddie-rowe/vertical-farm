...
export interface DeviceStatusProps
  ...
  deviceType?: "light" | "switch" | "sensor" | "fan" | "pump" | "valve" | "climate" | "generic"
...
function DeviceStatus({
  ...
  const statusText = isOnline 
    ? deviceState === "on" ? "Active" : deviceState === "off" ? "Inactive" : "Unknown"
    : "Offline"
  
  const label = ariaLabel || `${deviceName || deviceType} - ${statusText}${!isOnline ? " (Disconnected)" : ""}`
  ...
  if (variant === "compact") {
    return (
      <div className={cn("flex items-center gap-1", className)} {...props}>
        <StatusIndicator
          status={getDeviceStatus()}
          size={size === "sm" ? "xs" : size === "lg" || size === "xl" ? "md" : "sm"}
        />
        {showDeviceIcon && (
          <span className={cn(
