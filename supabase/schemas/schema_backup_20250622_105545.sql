...
CREATE OR REPLACE FUNCTION "public"."search_devices"("search_term" "text", "farm_uuid" "uuid" DEFAULT NULL::"uuid") RETURNS TABLE("device_id" "uuid", "entity_id" "text", "friendly_name" "text", "device_type" "text", "location_description" "text", "farm_name" "text", "relevance_score" numeric)
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
  RETURN QUERY
...
CREATE TABLE IF NOT EXISTS "public"."grow_alerts" (
    ...
    "created_at" timestamp with time zone DEFAULT "now"(),
    CONSTRAINT "grow_alerts_severity_check" CHECK ((("severity")::"text" = ANY ((ARRAY['info'::character varying, 'warning'::character varying, 'error'::character varying, 'critical'::character varying])::"text"[])))
);
...
CREATE TABLE IF NOT EXISTS "public"."grow_locations" (
    ...
    "created_at" timestamp with time zone DEFAULT "now"(),
    "updated_at" timestamp with time zone DEFAULT "now"(),
    CONSTRAINT "grow_locations_type_check" CHECK ((("type")::"text" = ANY ((ARRAY['row'::character varying, 'rack'::character varying, 'shelf'::character varying])::"text"[])))
);
...
CREATE TABLE IF NOT EXISTS "public"."grows" (
    ...
    "created_at" timestamp with time zone DEFAULT "now"(),
    "updated_at" timestamp with time zone DEFAULT "now"(),
    CONSTRAINT "grows_status_check" CHECK ((("status")::"text" = ANY ((ARRAY['planned'::character varying, 'active'::character varying, 'harvested'::character varying, 'failed'::character varying, 'paused'::character varying])::"text"[])))
);
...
CREATE OR REPLACE VIEW "public"."current_grows_view" AS
 SELECT "g"."id",
  ...
  WHERE (("g"."status")::"text" = ANY ((ARRAY['active'::character varying, 'planned'::character varying])::"text"[]))
...
CREATE OR REPLACE VIEW "public"."grow_analytics_view" AS
 SELECT "g"."id",
  ...
  WHERE (("g"."status")::"text" = ANY ((ARRAY['harvested'::character varying, 'failed'::character varying])::"text"[]))
...
ALTER TABLE ONLY "public"."grow_stages"
    ADD CONSTRAINT "grow_stages_order_index_key" UNIQUE ("order_index");
...
ALTER TABLE ONLY "public"."racks"
    ADD CONSTRAINT "racks_row_id_name_key" UNIQUE ("row_id", "name");



ALTER TABLE ONLY "public"."racks"
    ADD CONSTRAINT "racks_row_id_position_in_row_key" UNIQUE ("row_id", "position_in_row");
...
ALTER TABLE ONLY "public"."rows"
    ADD CONSTRAINT "rows_farm_id_name_key" UNIQUE ("farm_id", "name");



ALTER TABLE ONLY "public"."rows"
    ADD CONSTRAINT "rows_farm_id_position_key" UNIQUE ("farm_id", "position");
...
ALTER TABLE ONLY "public"."shelves"
    ADD CONSTRAINT "shelves_rack_id_name_key" UNIQUE ("rack_id", "name");



ALTER TABLE ONLY "public"."shelves"
    ADD CONSTRAINT "shelves_rack_id_position_in_rack_key" UNIQUE ("rack_id", "position_in_rack");
