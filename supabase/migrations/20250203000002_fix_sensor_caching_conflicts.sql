...
-- Remove sensor_readings from realtime publication
...
CREATE INDEX idx_sensor_alerts_device_assignment_id ON public.sensor_alerts(device_assignment_id);
CREATE INDEX idx_sensor_alerts_alert_type ON public.sensor_alerts(alert_type);
CREATE INDEX idx_sensor_alerts_severity ON public.sensor_alerts(severity);
CREATE INDEX idx_sensor_alerts_acknowledged ON public.sensor_alerts(acknowledged);
CREATE INDEX idx_sensor_alerts_resolved ON public.sensor_alerts(resolved);
CREATE INDEX idx_sensor_alerts_created_at ON public.sensor_alerts(created_at DESC);
...
CREATE TABLE public.sensor_alert_thresholds (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  device_assignment_id UUID NOT NULL REFERENCES public.device_assignments(id) ON DELETE CASCADE,
...
CREATE INDEX idx_sensor_alert_thresholds_device_assignment_id ON public.sensor_alert_thresholds(device_assignment_id);
CREATE INDEX idx_sensor_alert_thresholds_sensor_type ON public.sensor_alert_thresholds(sensor_type);
CREATE INDEX idx_sensor_alert_thresholds_enabled ON public.sensor_alert_thresholds(enabled);
...
WHERE da.device_class = 'temperature'
...
SELECT 
  ...
  95.0  -- critical_max
FROM public.device_assignments da
WHERE da.device_class = 'humidity'
ON CONFLICT (device_assignment_id, sensor_type) DO NOTHING;
