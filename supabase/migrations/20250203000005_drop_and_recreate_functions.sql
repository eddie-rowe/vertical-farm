-- Drop and recreate functions to fix manager_id references
-- This migration drops existing functions and recreates them with user_id instead of manager_id
...
DROP FUNCTION IF EXISTS public.get_active_schedules_with_progress(uuid);
DROP FUNCTION IF EXISTS public.get_harvest_analytics(uuid, date, date);
DROP FUNCTION IF EXISTS public.get_user_storage_usage(uuid);

-- Recreate search_devices function with user_id
...
-- Recreate get_farm_statistics function with user_id
...
-- Recreate get_device_status_summary function with user_id
...
-- Recreate get_active_schedules_with_progress function with user_id
...
-- Recreate get_harvest_analytics function with user_id
...
-- Recreate get_user_storage_usage function with user_id
