...
async def get_integration_status(
    ...
    try:
        ...
        logger.error(
            f"Failed to get integration status for user {current_user.id}: {e}"
        )
        raise HTTPException(
...
async def health_check(
    ...
    try:
        ...
        return HealthCheckResponse(
            healthy=False, services={"home_assistant": False}, version=None
        )
    ...
    try:
        ...
        for device in devices:
            try:
                ...
                friendly_name = (
                    device.get("name")
                    or device.get("attributes", {}).get("friendly_name")
                    or entity_id
                )
                ...
                device_model = HomeAssistantDevice(
                    ...
                    unit_of_measurement=device.get("attributes", {}).get(
                        "unit_of_measurement"
                    ),
                )
                ...
                logger.warning(
                    f"Failed to parse device {device.get('entity_id', 'unknown')} for user {user_id}: {e}"
                )
                continue
        ...
        return DeviceListResponse(
            devices=device_models, total_count=len(device_models), device_type=None
        )
    ...
    try:
        ...
        for entity_id in request.entity_ids:
            try:
                ...
                device_name = (
                    device.get("attributes", {}).get("friendly_name")
                    or device.get("name")
                    or entity_id
                )
                ...
                if request.update_existing:
                    result = (
                        await db.table("home_assistant_devices")
                        .upsert(device_data, on_conflict="user_id,entity_id")
                        .execute()
                    )
...
async def get_device_details(
    ...
    try:
        ...
        if not device:
            return DeviceDetailsResponse(device=None, found=False, cached=False)
        ...
        return DeviceDetailsResponse(device=device_model, found=True, cached=use_cache)

    except HTTPException:
        raise
    except Exception as e:
        logger.error(
            f"Failed to get device {entity_id} for user {current_user.id}: {e}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def control_device(
    ...
    try:
        ...
        result = await user_ha_service.control_device(
            user_id, request.entity_id, request.action
        )
        ...
        logger.error(
            f"Failed to control device {request.entity_id} for user {current_user.id}: {e}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
        ...
        for sensor in sensors:
            try:
                sensor_model = SensorData(
                    ...
                    unit_of_measurement=sensor.get("attributes", {}).get(
                        "unit_of_measurement"
                    ),
                ...
                logger.warning(
                    f"Failed to parse sensor {sensor.get('entity_id', 'unknown')} for user {user_id}: {e}"
                )
                continue

        return SensorDataResponse(sensors=sensor_models, total_count=len(sensor_models))
...
async def assign_device_to_location(
    ...
    try:
        ...
        if not device:
            raise HTTPException(
                status_code=404, detail="Device not found in Home Assistant"
            )
        ...
        result = (
            await db.table("device_assignments")
            .upsert(assignment_data, on_conflict="user_id,entity_id")
            .execute()
        )

        if not result.data:
            raise HTTPException(
                status_code=500, detail="Failed to store device assignment"
            )
...
async def get_device_assignments(
    ...
    try:
        ...
        if result.data is None:
            ...
            raise HTTPException(
                status_code=500, detail="Failed to retrieve device assignments"
            )

        logger.info(f"Query returned {len(result.data)} results")

        return {"assignments": result.data, "count": len(result.data)}
...
async def get_farm_assigned_devices(
    ...
    try:
        ...
        assignments_response = await get_device_assignments(
            farm_id=farm_id, current_user=current_user, db=db
        )
        ...
        for assignment in assignments:
            ...
            try:
                device_state = await ha_service.get_user_device(
                    str(current_user.id), entity_id
                )
                devices_with_states.append(
                    {**assignment, "current_state": device_state}
                )
            except Exception as e:
                logger.warning(f"Could not get state for device {entity_id}: {str(e)}")
                devices_with_states.append(
                    {**assignment, "current_state": None, "state_error": str(e)}
                )
...
async def get_user_configs(
    ...
    try:
        ...
        result = (
            await db.from_("user_home_assistant_configs")
            .select("*")
...
async def create_user_config(
    ...
    try:
        ...
        existing_names = (
            [config["name"] for config in existing_configs.data]
            if existing_configs.data
            else []
        )
        existing_defaults = (
            [config for config in existing_configs.data if config.get("is_default")]
            if existing_configs.data
            else []
        )
        ...
        try:
            ...
            if "unique_default_per_user" in error_message or "23505" in error_message:
                ...
                logger.warning(
                    f"Unique constraint violation, retrying with is_default=False: {error_message}"
                )
