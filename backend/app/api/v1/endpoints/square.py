...
class SquareService:
    ...
    async def _get_cache_entry(self, user_id: str, entity_type: str, cache_key: str, supabase) -> Optional[Dict[str, Any]]:
        """Get a cache entry from the database"""
        try:
            table_name = f"square_cache_{entity_type}"
            response = await supabase.table(table_name).select("*").eq("user_id", user_id).eq("cache_key", cache_key).single().execute()
            ...
            if response.data:
                ...
                if datetime.utcnow() <= cached_at.replace(tzinfo=None) + timedelta(minutes=ttl_minutes):
                    return response.data
                else:
                    # Cache expired, delete it
                    await self._delete_cache_entry(user_id, entity_type, cache_key, supabase)
            
            return None
        except Exception as e:
            logger.error(f"Error getting cache entry for {entity_type}/{cache_key}: {e}")
            return None
    
    async def _set_cache_entry(self, user_id: str, entity_type: str, cache_key: str, data: Any, ttl_minutes: int, supabase) -> bool:
        ...
        try:
            ...
            if hasattr(data, 'dict'):
                # Pydantic model
                json_data = [item.dict() if hasattr(item, 'dict') else item for item in data] if isinstance(data, list) else data.dict()
            ...
            response = await supabase.table(table_name).upsert(cache_entry, on_conflict="user_id,cache_key").execute()
            
            # Log the sync operation
            await self._log_sync_operation(user_id, entity_type, "cache_set", f"Cached {cache_key}", supabase)
            
            return True
        except Exception as e:
            logger.error(f"Error setting cache entry for {entity_type}/{cache_key}: {e}")
            return False
    
    async def _delete_cache_entry(self, user_id: str, entity_type: str, cache_key: str, supabase) -> bool:
        """Delete a cache entry from the database"""
        try:
            table_name = f"square_cache_{entity_type}"
            await supabase.table(table_name).delete().eq("user_id", user_id).eq("cache_key", cache_key).execute()
            return True
        except Exception as e:
            logger.error(f"Error deleting cache entry for {entity_type}/{cache_key}: {e}")
            return False
    
    async def _invalidate_cache_for_user(self, user_id: str, entity_type: Optional[str] = None, supabase = None) -> bool:
        """Invalidate cache entries for a user (optionally for specific entity type)"""
        try:
            if entity_type:
                # Invalidate specific entity type
                table_name = f"square_cache_{entity_type}"
                await supabase.table(table_name).delete().eq("user_id", user_id).execute()
            else:
                # Invalidate all cache types for user
                cache_types = ["locations", "products", "customers", "orders", "payments"]
                for cache_type in cache_types:
                    table_name = f"square_cache_{cache_type}"
                    await supabase.table(table_name).delete().eq("user_id", user_id).execute()
            
            await self._log_sync_operation(user_id, entity_type or "all", "cache_invalidate", f"Invalidated cache for {entity_type or 'all entities'}", supabase)
            return True
        except Exception as e:
            logger.error(f"Error invalidating cache for user {user_id}: {e}")
            return False
    
    async def _log_sync_operation(self, user_id: str, entity_type: str, operation: str, details: str, supabase) -> None:
    ...
    async def _get_cache_key(self, config: SquareConfig, additional_params: Optional[str] = None) -> str:
    ...
    async def verify_webhook_signature(self, payload: str, signature: str, signature_key: str) -> bool:
    ...
    async def process_webhook_event(self, user_id: str, payload: SquareWebhookPayload, signature_header: str, supabase) -> bool:
        ...
        try:
            ...
            logger.info(f"Successfully processed webhook event {payload.event_id} for user {user_id}")
            return True
    ...
    def _get_cache_types_for_event(self, event_type: str) -> List[str]:
        ...
        event_cache_mapping = {
            ...
            "inventory.count.updated": ["products"],  # Inventory affects product availability
        }
        
        return event_cache_mapping.get(event_type, [])

    async def _store_webhook_event(self, event_data: SquareWebhookEventCreate, supabase) -> str:
        """Store webhook event in database."""
        try:
            result = await supabase.table("square_webhook_events").insert({
    ...
    async def _update_webhook_event_status(self, event_id: str, status: str, cache_invalidated: bool = False, error_message: Optional[str] = None, supabase = None) -> None:
        ...
        try:
            ...
            await supabase.table("square_webhook_events").update(update_data).eq("event_id", event_id).execute()
            
        except Exception as e:
            logger.error(f"Error updating webhook event status: {e}")

    async def get_webhook_config(self, user_id: str, supabase) -> Optional[Dict[str, Any]]:
        """Get webhook configuration for a user."""
        try:
            result = await supabase.table("square_webhooks").select("*").eq("user_id", user_id).execute()
    ...
    async def create_webhook_config(self, user_id: str, webhook_data: SquareWebhookCreate, supabase) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await supabase.table("square_webhooks").insert(config_data).execute()
    ...
    async def register_webhook_with_square(self, config: SquareConfig, webhook_url: str, event_types: List[str]) -> WebhookRegistrationResponse:
    ...
    async def invalidate_cache_manually(self, user_id: str, cache_types: List[str], reason: Optional[str] = None, supabase = None) -> CacheInvalidationResponse:
        ...
        try:
            ...
            for cache_type in cache_types:
                if cache_type == "all":
                    ...
                    invalidated_caches = ["locations", "products", "customers", "orders", "payments"]
                    break
                else:
    ...
    def is_configured(self, config: Optional[SquareConfig] = None) -> bool:
        ...
        return bool(
            check_config and 
            check_config.access_token and 
            check_config.application_id
        )
    ...
    async def get_user_configs(self, user_id: str, supabase) -> List[SquareConfig]:
        ...
        try:
            response = await supabase.table("user_square_configs").select("*").eq("user_id", user_id).execute()
            ...
            raise HTTPException(status_code=500, detail="Failed to fetch Square configurations")
    
    async def get_config_by_id(self, user_id: str, config_id: str, supabase) -> Optional[SquareConfig]:
        """Get a specific Square configuration by ID"""
        try:
            response = await supabase.table("user_square_configs").select("*").eq("user_id", user_id).eq("id", config_id).single().execute()
    ...
    async def create_config(self, user_id: str, config_data: SquareConfigCreate, supabase) -> SquareConfig:
        ...
        try:
            ...
            response = await supabase.table("user_square_configs").insert(insert_data).execute()
            ...
            if response.data:
                ...
                try:
                    ...
                    webhook_config = await self.create_webhook_config(user_id, webhook_config_data, supabase)
                    ...
                    logger.info(f"Automatically registered webhook for user {user_id} with Square config {config.id}")
                    
                except Exception as webhook_error:
                    # Log webhook registration failure but don't fail the config creation
                    logger.warning(f"Failed to auto-register webhook for user {user_id}: {webhook_error}")
                    ...
                    try:
                        ...
                        logger.error(f"Failed to update webhook status after error: {update_error}")
                
                return config
            else:
                raise HTTPException(status_code=400, detail="Failed to create Square configuration")
                
        except Exception as e:
            logger.error(f"Error creating Square config: {e}")
            if "duplicate key value violates unique constraint" in str(e):
                raise HTTPException(status_code=400, detail="A configuration with this name or application ID already exists")
            raise HTTPException(status_code=500, detail="Failed to create Square configuration")
    
    async def update_config(self, user_id: str, config_id: str, config_data: SquareConfigUpdate, supabase) -> Optional[SquareConfig]:
        ...
        try:
            ...
            response = await supabase.table("user_square_configs").update(update_data).eq("user_id", user_id).eq("id", config_id).execute()
            ...
            raise HTTPException(status_code=500, detail="Failed to update Square configuration")
    
    async def delete_config(self, user_id: str, config_id: str, supabase) -> bool:
        """Delete a Square configuration"""
        try:
            response = await supabase.table("user_square_configs").delete().eq("user_id", user_id).eq("id", config_id).execute()
            
            return len(response.data) > 0
                
        except Exception as e:
            logger.error(f"Error deleting Square config {config_id}: {e}")
            raise HTTPException(status_code=500, detail="Failed to delete Square configuration")
    
    async def test_connection(self, config: SquareConfig, user_id: Optional[str] = None, supabase = None) -> SquareConnectionStatus:
                ...
                if response.status_code == 200:
                    ...
                    if user_id and supabase:
                        try:
                            ...
                            webhook_result = await supabase.table("square_webhooks").select("*").eq("user_id", user_id).execute()
                            if webhook_result.data:
                                ...
                                events_result = await supabase.table("square_webhook_events").select("*").eq("user_id", user_id).execute()
                                webhook_events_received = len(events_result.data)
                                
                                if events_result.data:
                                    # Get last event timestamp
                                    sorted_events = sorted(events_result.data, key=lambda x: x["created_at"], reverse=True)
                                    webhook_last_event = datetime.fromisoformat(sorted_events[0]["created_at"].replace('Z', '+00:00'))
                        except Exception as e:
    ...
    async def get_locations(self, config: SquareConfig, user_id: Optional[str] = None, supabase = None) -> List[SquareLocation]:
        ...
        if user_id and supabase:
            try:
                ...
                cached_data = await self._get_cache_entry(user_id, "locations", cache_key, supabase)
                ...
                if cached_data:
                    ...
                    await self._log_sync_operation(user_id, "locations", "cache_hit", f"Retrieved {len(locations)} locations from cache", supabase)
                    return locations
            except Exception as e:
                ...
                if response.status_code == 200:
                    ...
                    if user_id and supabase:
                        try:
                            ...
                            await self._set_cache_entry(user_id, "locations", cache_key, cache_data, 60, supabase)  # 60 minutes TTL
                            await self._log_sync_operation(user_id, "locations", "api_fetch", f"Fetched and cached {len(locations)} locations", supabase)
                        except Exception as e:
                    ...
                    raise HTTPException(status_code=response.status_code, detail="Failed to fetch locations from Square")
                    
        except httpx.RequestError as e:
            logger.error(f"Network error calling Square API: {e}")
            raise HTTPException(status_code=503, detail="Unable to connect to Square API")
        except Exception as e:
            logger.error(f"Error fetching Square locations: {e}")
            raise HTTPException(status_code=500, detail="Failed to fetch locations")

    async def get_catalog_items(self, config: SquareConfig, user_id: Optional[str] = None, supabase = None) -> List[SquareProduct]:
        ...
        if user_id and supabase:
            ...
            cached_data = await self._get_cache_entry(user_id, "products", cache_key, supabase)
            
            if cached_data:
                logger.info(f"Retrieved {len(cached_data['data'])} catalog items from cache for user {user_id}")
                ...
                if response.status_code == 200:
                    ...
                    if user_id and supabase and products:
                        try:
                            ...
                            await self._set_cache_entry(user_id, "products", cache_key, cache_data, 60, supabase)  # 1 hour TTL
                            await self._log_sync_operation(user_id, "products", "api_fetch", f"Fetched and cached {len(products)} catalog items", supabase)
                        except Exception as e:
                    ...
                    logger.error(f"Square API error: {response.status_code} - {response.text}")
                    raise HTTPException(status_code=response.status_code, detail="Failed to fetch products from Square")
    ...
    async def get_customers(self, config: SquareConfig, user_id: Optional[str] = None, supabase = None) -> List[SquareCustomer]:
        ...
        if user_id and supabase:
            ...
            cached_data = await self._get_cache_entry(user_id, "customers", cache_key, supabase)
            
            if cached_data:
                logger.info(f"Retrieved {len(cached_data['data'])} customers from cache for user {user_id}")
                ...
                if response.status_code == 200:
                    ...
                    if user_id and supabase:
                        try:
                            ...
                            await self._set_cache_entry(user_id, "customers", cache_key, cache_data, 60, supabase)  # 60 minutes TTL
                            await self._log_sync_operation(user_id, "customers", "api_fetch", f"Fetched and cached {len(customers)} customers", supabase)
                        except Exception as e:
                    ...
                    logger.error(f"Square API error: {response.status_code} - {response.text}")
                    raise HTTPException(status_code=response.status_code, detail="Failed to fetch customers from Square")
    ...
    async def get_orders(self, config: SquareConfig, location_id: Optional[str] = None, user_id: Optional[str] = None, supabase = None) -> List[SquareOrder]:
        ...
        # Try cache first if user_id and supabase are provided
        if user_id and supabase:
            cache_key = await self._get_cache_key(config, f"orders_{location_id}" if location_id else "orders")
            cached_data = await self._get_cache_entry(user_id, "orders", cache_key, supabase)
            
            if cached_data:
                logger.info(f"Retrieved {len(cached_data['data'])} orders from cache for user {user_id}")
                ...
                if response.status_code == 200:
                    ...
                    for order_data in data.get("orders", []):
                        order = SquareOrder(
                            ...
                            created_at=datetime.fromisoformat(order_data["created_at"].replace("Z", "+00:00")) if order_data.get("created_at") else None,
                    ...
                    if user_id and supabase:
                        try:
                            ...
                            await self._set_cache_entry(user_id, "orders", cache_key, cache_data, 30, supabase)  # 30 minutes TTL for orders
                            await self._log_sync_operation(user_id, "orders", "api_fetch", f"Fetched and cached {len(orders)} orders", supabase)
                        except Exception as e:
                    ...
                    logger.error(f"Square API error: {response.status_code} - {response.text}")
                    raise HTTPException(status_code=response.status_code, detail="Failed to fetch orders from Square")
    ...
    async def get_payments(self, config: SquareConfig, location_id: Optional[str] = None, user_id: Optional[str] = None, supabase = None) -> List[SquarePayment]:
        ...
        # Try cache first if user_id and supabase are provided
        if user_id and supabase:
            cache_key = await self._get_cache_key(config, f"payments_{location_id}" if location_id else "payments")
            cached_data = await self._get_cache_entry(user_id, "payments", cache_key, supabase)
            
            if cached_data:
                logger.info(f"Retrieved {len(cached_data['data'])} payments from cache for user {user_id}")
                ...
                if response.status_code == 200:
                    ...
                    for payment_data in data.get("payments", []):
                        payment = SquarePayment(
                            ...
                            created_at=datetime.fromisoformat(payment_data["created_at"].replace("Z", "+00:00")) if payment_data.get("created_at") else None
                        )
                    ...
                    if user_id and supabase:
                        try:
                            ...
                            await self._set_cache_entry(user_id, "payments", cache_key, cache_data, 30, supabase)  # 30 minutes TTL for payments
                            await self._log_sync_operation(user_id, "payments", "api_fetch", f"Fetched and cached {len(payments)} payments", supabase)
                        except Exception as e:
                    ...
                    logger.error(f"Square API error: {response.status_code} - {response.text}")
                    raise HTTPException(status_code=response.status_code, detail="Failed to fetch payments from Square")
    ...
    async def get_inventory_counts(self, config: SquareConfig, location_ids: Optional[List[str]] = None) -> List[SquareInventoryCount]:
                ...
                if response.status_code == 200:
                    ...
                    for count_data in data.get("counts", []):
                        inventory_count = SquareInventoryCount(
                            ...
                            catalog_object_type=count_data.get("catalog_object_type", ""),
                            state=count_data.get("state", ""),
                            location_id=count_data.get("location_id", ""),
                            quantity=count_data.get("quantity"),
                            calculated_at=datetime.fromisoformat(count_data["calculated_at"].replace("Z", "+00:00")) if count_data.get("calculated_at") else None
                        )
                    ...
                    logger.error(f"Square API error: {response.status_code} - {response.text}")
                    raise HTTPException(status_code=response.status_code, detail="Failed to fetch inventory from Square")
    ...
    async def get_refunds(self, config: SquareConfig, location_id: Optional[str] = None) -> List[SquareRefund]:
                ...
                if response.status_code == 200:
                    ...
                    for refund_data in data.get("refunds", []):
                        refund = SquareRefund(
                            ...
                            created_at=datetime.fromisoformat(refund_data["created_at"].replace("Z", "+00:00")) if refund_data.get("created_at") else None,
                            updated_at=datetime.fromisoformat(refund_data["updated_at"].replace("Z", "+00:00")) if refund_data.get("updated_at") else None
                        )
                    ...
                    logger.error(f"Square API error: {response.status_code} - {response.text}")
                    raise HTTPException(status_code=response.status_code, detail="Failed to fetch refunds from Square")
                ...
                if response.status_code == 200:
                    ...
                    for dispute_data in data.get("disputes", []):
                        dispute = SquareDispute(
                            ...
                            due_at=datetime.fromisoformat(dispute_data["due_at"].replace("Z", "+00:00")) if dispute_data.get("due_at") else None,
                            ...
                            created_at=datetime.fromisoformat(dispute_data["created_at"].replace("Z", "+00:00")) if dispute_data.get("created_at") else None,
                            updated_at=datetime.fromisoformat(dispute_data["updated_at"].replace("Z", "+00:00")) if dispute_data.get("updated_at") else None
                        )
                    ...
                    logger.error(f"Square API error: {response.status_code} - {response.text}")
                    raise HTTPException(status_code=response.status_code, detail="Failed to fetch disputes from Square")
    ...
    async def get_subscriptions(self, config: SquareConfig, location_id: Optional[str] = None) -> List[SquareSubscription]:
                ...
                if response.status_code == 200:
                    ...
                    for subscription_data in data.get("subscriptions", []):
                        subscription = SquareSubscription(
                            ...
                            invoice_request_method=subscription_data.get("invoice_request_method"),
                            charge_through_date=subscription_data.get("charge_through_date"),
                            charged_through_date=subscription_data.get("charged_through_date"),
                            paid_until_date=subscription_data.get("paid_until_date"),
                            created_at=datetime.fromisoformat(subscription_data["created_at"].replace("Z", "+00:00")) if subscription_data.get("created_at") else None,
                    ...
                    logger.error(f"Square API error: {response.status_code} - {response.text}")
                    raise HTTPException(status_code=response.status_code, detail="Failed to fetch subscriptions from Square")
    ...
    async def get_invoices(self, config: SquareConfig, location_id: Optional[str] = None) -> List[SquareInvoice]:
                ...
                if response.status_code == 200:
                    ...
                    for invoice_data in data.get("invoices", []):
                        invoice = SquareInvoice(
                            ...
                            scheduled_at=datetime.fromisoformat(invoice_data["scheduled_at"].replace("Z", "+00:00")) if invoice_data.get("scheduled_at") else None,
                            public_url=invoice_data.get("public_url"),
                            status=invoice_data.get("status", "UNKNOWN"),
                            timezone=invoice_data.get("timezone"),
                            created_at=datetime.fromisoformat(invoice_data["created_at"].replace("Z", "+00:00")) if invoice_data.get("created_at") else None,
                            updated_at=datetime.fromisoformat(invoice_data["updated_at"].replace("Z", "+00:00")) if invoice_data.get("updated_at") else None
                        )
                    ...
                    logger.error(f"Square API error: {response.status_code} - {response.text}")
                    raise HTTPException(status_code=response.status_code, detail="Failed to fetch invoices from Square")
                ...
                if response.status_code == 200:
                    ...
                    for member_data in data.get("team_members", []):
                        team_member = SquareTeamMember(
                            ...
                            created_at=datetime.fromisoformat(member_data["created_at"].replace("Z", "+00:00")) if member_data.get("created_at") else None,
                            updated_at=datetime.fromisoformat(member_data["updated_at"].replace("Z", "+00:00")) if member_data.get("updated_at") else None,
                    ...
                    logger.error(f"Square API error: {response.status_code} - {response.text}")
                    raise HTTPException(status_code=response.status_code, detail="Failed to fetch team members from Square")
    ...
    async def get_labor(self, config: SquareConfig, location_id: Optional[str] = None) -> List[SquareLabor]:
                ...
                if response.status_code == 200:
                    ...
                    for shift_data in data.get("shifts", []):
                        labor = SquareLabor(
                            ...
                            start_at=datetime.fromisoformat(shift_data["start_at"].replace("Z", "+00:00")) if shift_data.get("start_at") else None,
                            end_at=datetime.fromisoformat(shift_data["end_at"].replace("Z", "+00:00")) if shift_data.get("end_at") else None,
                            wage=SquareWage(**shift_data["wage"]) if shift_data.get("wage") else None,
                            teamMember_id=shift_data.get("team_member_id"),
                            declared_cash_tip_money=shift_data.get("declared_cash_tip_money"),
                            version=shift_data.get("version"),
                            created_at=datetime.fromisoformat(shift_data["created_at"].replace("Z", "+00:00")) if shift_data.get("created_at") else None,
                            updated_at=datetime.fromisoformat(shift_data["updated_at"].replace("Z", "+00:00")) if shift_data.get("updated_at") else None
                        )
                    ...
                    logger.error(f"Square API error: {response.status_code} - {response.text}")
                    raise HTTPException(status_code=response.status_code, detail="Failed to fetch labor data from Square")
                ...
                if response.status_code == 200:
                    ...
                    for merchant_data in data.get("merchant", []):
                        merchant = SquareMerchant(
                            ...
                            created_at=datetime.fromisoformat(merchant_data["created_at"].replace("Z", "+00:00")) if merchant_data.get("created_at") else None
                        )
                    ...
                    logger.error(f"Square API error: {response.status_code} - {response.text}")
                    raise HTTPException(status_code=response.status_code, detail="Failed to fetch merchants from Square")
    ...
    async def get_payouts(self, config: SquareConfig, location_id: Optional[str] = None) -> List[SquarePayout]:
                ...
                if response.status_code == 200:
                    ...
                    for payout_data in data.get("payouts", []):
                        payout = SquarePayout(
                            ...
                            created_at=datetime.fromisoformat(payout_data["created_at"].replace("Z", "+00:00")) if payout_data.get("created_at") else None,
                            updated_at=datetime.fromisoformat(payout_data["updated_at"].replace("Z", "+00:00")) if payout_data.get("updated_at") else None,
                    ...
                    logger.error(f"Square API error: {response.status_code} - {response.text}")
                    raise HTTPException(status_code=response.status_code, detail="Failed to fetch payouts from Square")
...
async def receive_square_webhook(
    request: Request,
    db = Depends(get_async_supabase_client)
):
    ...
    try:
        ...
        user_result = await db.table("user_square_configs").select("user_id").eq("merchant_id", payload.merchant_id).execute()
...
async def register_webhook(
    ...
    try:
        ...
        webhook_config = await service.create_webhook_config(current_user.id, webhook_config_data, db)
...
async def get_webhook_config(
    ...
    try:
        ...
        logger.error(f"Error getting webhook config: {e}")
        raise HTTPException(status_code=500, detail="Failed to get webhook configuration")
...
async def get_webhook_events(
    ...
    try:
        result = await db.table("square_webhook_events").select("*").eq("user_id", current_user.id).order("created_at", desc=True).range(offset, offset + limit - 1).execute()
...
async def get_webhook_health(
    ...
    try:
        # Get webhook config
        webhook_result = await db.table("square_webhooks").select("*").eq("user_id", current_user.id).execute()
        ...
        events_result = await db.table("square_webhook_events").select("*").eq("user_id", current_user.id).execute()
        ...
        if events_result.data:
            ...
            sorted_events = sorted(events_result.data, key=lambda x: x["created_at"], reverse=True)
            last_event = datetime.fromisoformat(sorted_events[0]["created_at"].replace('Z', '+00:00'))
...
async def update_square_config(
    ...
    config = await square_service.update_config(str(current_user.id), config_id, config_data, db)
    if not config:
        raise HTTPException(status_code=404, detail="Square configuration not found")
    return config
...
async def get_square_orders(
    ...
    return await square_service.get_orders(config, location_id, str(current_user.id), db)
...
async def get_square_payments(
    ...
    return await square_service.get_payments(config, location_id, str(current_user.id), db)
...
async def get_square_status(
    config_id: Optional[str] = Query(None, description="Configuration ID to check status"),
    current_user: User = Depends(get_current_user),
    db = Depends(get_async_rls_client)
):
    ...
    if config_id:
        ...
        active_config = next((c for c in configs if c.is_active), configs[0] if configs else None)
        if active_config:
            return await square_service.test_connection(active_config, str(current_user.id), db)
        else:
            return SquareConnectionStatus(
                connected=False,
                environment="sandbox",
