...
async def get_latest_sensor_readings(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fetching sensor readings: {str(e)}"
        )
...
async def get_sensor_history(
    ...
    sensor_type: str,
    hours: int = Query(
        24, description="Number of hours of history to retrieve", ge=1, le=168
    ),
    current_user: User = Depends(get_current_user),
    cache_service: SensorCacheService = Depends(get_sensor_cache_service),
):
    ...
    try:
        ...
        if not readings:
            raise HTTPException(
                status_code=404,
                detail="No sensor data found for the specified device and sensor type",
            )
...
async def get_sensor_aggregates(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fetching sensor aggregates: {str(e)}"
        )
...
async def get_species_data(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fetching species data: {str(e)}"
        )
...
async def get_plant_varieties_data(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fetching plant varieties data: {str(e)}"
        )
...
async def get_grow_recipes_data(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fetching grow recipes data: {str(e)}"
        )
