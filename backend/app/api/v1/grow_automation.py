...
async def initialize_grow_automation(
    ...
    try:
        ...
        grow_response = await supabase.table('grows').select('id, created_by').eq('id', grow_id).eq('created_by', current_user.id).single().execute()
...
async def get_grow_automation(
    ...
    try:
        ...
        # Get automation data
        rules_response = await supabase.table('grow_automation_rules').select('*').eq('grow_id', grow_id).execute()
        schedules_response = await supabase.table('grow_automation_schedules').select('*').eq('grow_id', grow_id).execute()
        conditions_response = await supabase.table('grow_automation_conditions').select('*').eq('grow_id', grow_id).execute()
        executions_response = await supabase.table('grow_automation_executions').select('*').eq('grow_id', grow_id).order('executed_at', desc=True).limit(20).execute()
        
        # Get device assignments
        device_assignments = await grow_automation_service._get_grow_device_assignments(grow_id)
        
        # Get status
        status_data = await grow_automation_service.get_grow_automation_status(grow_id, current_user.id)
...
async def create_automation_schedule(
    ...
    try:
        ...
        response = await supabase.table('grow_automation_schedules').insert(schedule_dict).execute()
...
async def update_automation_schedule(
    ...
    try:
        ...
        schedule_response = await supabase.table('grow_automation_schedules').select(
            '*, grows!inner(created_by)'
        ).eq('id', schedule_id).single().execute()
        
        if not schedule_response.data or schedule_response.data['grows']['created_by'] != current_user.id:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
        ...
        response = await supabase.table('grow_automation_schedules').update(update_dict).eq('id', schedule_id).execute()
...
async def delete_automation_schedule(
    ...
    try:
        ...
        # Delete schedule
        response = await supabase.table('grow_automation_schedules').delete().eq('id', schedule_id).execute()
...
async def create_automation_condition(
    ...
    try:
        ...
        response = await supabase.table('grow_automation_conditions').insert(condition_dict).execute()
...
async def get_automation_executions(
    ...
    try:
        ...
        # Get executions
        executions_response = await supabase.table('grow_automation_executions').select('*').eq('grow_id', grow_id).order('executed_at', desc=True).limit(limit).execute()
        
        return [AutomationExecutionResponse(**execution) for execution in executions_response.data or []]
...
async def toggle_grow_automation(
    ...
    try:
        ...
        update_response = await supabase.table('grows').update({
            'automation_enabled': enabled
        }).eq('id', grow_id).execute()
        ...
        if enabled:
            await grow_automation_service.initialize_grow_automation(grow_id, current_user.id)
        else:
...
async def manually_execute_automation(
    ...
    try:
        ...
        await supabase.table('grow_automation_executions').insert(execution_data).execute()
        
        return {
            'success': result.get('success', False),
            'message': 'Action executed successfully' if result.get('success') else 'Action failed',
            'result': result
        }
