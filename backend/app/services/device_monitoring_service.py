...
class DeviceMonitoringService:
    ...
    async def monitor_user_devices(self, user_id: str):
        ...
        try:
            ...
            entity_ids = [assignment['home_assistant_entity_id'] for assignment in device_assignments]
            ...
            await ha_client.subscribe_to_state_changes(
                ...
                callback=lambda entity_id, old_state, new_state: asyncio.create_task(
                    self.handle_device_state_change(user_id, entity_id, old_state, new_state)
                )
            )
    ...
    async def handle_device_state_change(self, user_id: str, entity_id: str, old_state: dict, new_state: dict):
    ...
    async def update_device_state_db(self, user_id: str, entity_id: str, state: str, attributes: dict):
        """Update device state in database"""
        try:
            supabase = self.db_service.get_supabase_client()
            
            # Use the stored procedure we created
            result = supabase.rpc('update_device_state', {
                'p_user_id': user_id,
    ...
    async def get_user_device_assignments(self, user_id: str) -> List[Dict]:
        ...
        try:
            ...
            result = supabase.table('device_assignments')\
                .select('*')\
                .eq('user_id', user_id)\
                .eq('is_active', True)\
                .execute()
    ...
    async def get_location_devices(self, user_id: str, location_id: str) -> List[Dict]:
        ...
        try:
            ...
            result = supabase.rpc('get_location_devices', {
                'p_user_id': user_id,
        ...
        try:
            ...
            try:
                if action['type'] == 'turn_on':
                    ...
                    success = await ha_client.set_brightness(entity_id, action['brightness'])
    ...
    async def get_device_current_state(self, user_id: str, entity_id: str) -> Optional[str]:
        ...
        try:
            ...
            supabase = self.db_service.get_supabase_client()
            result = supabase.table('device_states')\
                .select('state')\
                .eq('user_id', user_id)\
    ...
    async def log_device_control(self, user_id: str, entity_id: str, action_type: str, 
        ...
        try:
            ...
            result = supabase.rpc('log_device_control', {
                'p_user_id': user_id,
                'p_entity_id': entity_id,
    ...
    async def create_device_assignment(self, user_id: str, location_id: str, 
                                     entity_id: str, device_type: DeviceType,
        ...
        try:
            ...
            result = supabase.table('device_assignments').insert({
                'user_id': user_id,
    ...
    async def emergency_stop(self, user_id: str, location_ids: Optional[List[str]] = None,
                            device_types: Optional[List[DeviceType]] = None) -> Dict:
        """Emergency stop for devices"""
        try:
            # Get affected devices
            supabase = self.db_service.get_supabase_client()
            query = supabase.table('device_assignments').select('*').eq('user_id', user_id)
            ...
            for device in devices:
                ...
                try:
                    control_result = await self.control_device(user_id, entity_id, {'type': 'turn_off'})
