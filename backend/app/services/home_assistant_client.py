...
class HomeAssistantClient:
    ...
    def __init__(
        ...
        self.websocket_url = (
            websocket_url or f"{self.base_url.replace('http', 'ws')}/api/websocket"
        )
    ...
    def _setup_error_handling(self) -> None:
        ...
        global_error_handler.register_service(
            f"ha_rest_{id(self)}", rest_config, circuit_config
        )

        global_error_handler.register_service(
            f"ha_websocket_{id(self)}", websocket_config, circuit_config
        )
            ...
            async with self.session.get(f"{self.base_url}/api/") as response:
                if response.status == 401:
                    ...
                    raise ConnectionError(
                        f"Authentication test failed with status {response.status}"
                    )

                data = await response.json()
                logger.info(
                    f"Successfully authenticated with Home Assistant {data.get('version', 'unknown')}"
                )
                return data
        ...
        while True:
            try:
                ...
                if e.error_type == ErrorType.AUTHENTICATION_ERROR:
                    ...
                    logger.error(
                        "WebSocket connection failed permanently (circuit breaker open)"
                    )
                    break
                else:
        ...
        try:
            ...
            if hasattr(self, "headers") and self.headers:
                if "CF-Access-Client-Id" in self.headers:
                    ws_headers["CF-Access-Client-Id"] = self.headers[
                        "CF-Access-Client-Id"
                    ]
                if "CF-Access-Client-Secret" in self.headers:
                    ws_headers["CF-Access-Client-Secret"] = self.headers[
            ...
            try:
                self.websocket = await websockets.connect(
                    self.websocket_url, additional_headers=ws_headers
                )
            except TypeError:
                # Fallback for older versions
                try:
                    self.websocket = await websockets.connect(
                        self.websocket_url, extra_headers=ws_headers
                    )
                except TypeError:
                    # If all else fails, connect without custom headers
                    logger.warning(
                        "Unable to set WebSocket headers - connecting without authentication headers"
                    )
    ...
    async def _handle_command_result(self, data: dict) -> None:
        ...
        if success:
            ...
            logger.warning(
                f"Command {message_id} failed: {error.get('message', 'Unknown error')}"
            )
                    ...
                    async with session_to_use.get(
                        ...
                        if entity_type:
                            entities = [
                                e
                                for e in entities
                                if e.get("entity_id", "").startswith(f"{entity_type}.")
                            ]
    ...
    async def get_entity(
        ...
        if use_cache and entity_id in self.entity_cache:
            ...
            if (
                cache_time
                and (datetime.now() - cache_time).total_seconds() < self.cache_ttl
            ):
            ...
            try:
                ...
                async with self.throttler:
                    async with session_to_use.post(
                        ...
                        logger.debug(
                            f"Called service {domain}.{service} for {entity_id or 'all entities'}"
                        )
                        return result
    ...
    async def get_config(self) -> dict:
        ...
        return await global_error_handler.execute_with_retry(
            ...
            context={"operation": "get_config", "url": f"{self.base_url}/api/config"},
        )
    ...
    async def health_check(self) -> dict:
        ...
        health_status = {
            ...
            "cache_stats": {
                ...
                "cache_size_bytes": sum(
                    len(str(entity)) for entity in self.entity_cache.values()
                ),
            },
        }
        ...
        if rest_service in global_error_handler.metrics:
            health_status["services"]["rest_api"] = (
                global_error_handler.get_service_health(rest_service)
            )
        ...
        if websocket_service in global_error_handler.metrics:
            health_status["services"]["websocket"] = (
                global_error_handler.get_service_health(websocket_service)
            )
