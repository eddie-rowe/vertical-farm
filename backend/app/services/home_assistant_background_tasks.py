...
async def discover_home_assistant_devices(user_id: str, ha_config: Dict[str, Any]) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        logger.info(f"Device discovery completed for user {user_id}: {len(relevant_entities)} relevant entities found")
        return result
...
async def sync_device_states(user_id: str, ha_config: Dict[str, Any], entity_ids: List[str]) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        logger.info(f"State sync completed for user {user_id}: {len(states)} successful, {len(failed_entities)} failed")
        return result
...
async def health_check_home_assistant(user_id: str, ha_config: Dict[str, Any]) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        logger.info(f"Health check passed for user {user_id}: {response_time:.2f}s response time")
        return result
...
async def bulk_device_control(
    ...
    """
    logger.info(f"Starting bulk device control for user {user_id}, {len(device_commands)} commands")
    ...
    try:
        ...
        async def execute_command(command: Dict[str, Any]) -> Dict[str, Any]:
            try:
                ...
                logger.error(f"Command failed for {command.get('entity_id', 'unknown')}: {e}")
                return {
        ...
        results = await asyncio.gather(
            *[execute_command(cmd) for cmd in device_commands],
        ...
        logger.info(f"Bulk control completed for user {user_id}: {len(successful)} successful, {len(failed) + len(exceptions)} failed")
        return result
...
async def scheduled_device_action(
    ...
    Returns:
        Dict containing execution results
    """
    logger.info(f"Executing scheduled action '{schedule_name}' for user {user_id}, entity {entity_id}")
    ...
    try:
        ...
        logger.info(f"Scheduled action '{schedule_name}' completed successfully for user {user_id}")
        return result
        
    except Exception as e:
        logger.error(f"Scheduled action '{schedule_name}' failed for user {user_id}: {e}")
        raise
...
async def collect_device_history(
    ...
    """
    logger.info(f"Collecting device history for user {user_id}, {len(entity_ids)} entities, {hours_back}h back")
    ...
    try:
        ...
        logger.info(f"History collection completed for user {user_id}: {len(history_data)} successful, {len(failed_entities)} failed")
        return result
...
async def schedule_device_discovery(user_id: str, ha_config: Dict[str, Any], delay_minutes: int = 0) -> str:
    ...
    if delay_minutes > 0:
        ...
        return await supabase_bg_service.schedule_task(task_data, scheduled_for, priority)
    else:
        return await supabase_bg_service.enqueue_task(task_data, priority)


async def schedule_state_sync(user_id: str, ha_config: Dict[str, Any], entity_ids: List[str]) -> str:
...
async def schedule_health_check(user_id: str, ha_config: Dict[str, Any], delay_minutes: int = 0) -> str:
...
async def schedule_recurring_health_checks(user_id: str, ha_config: Dict[str, Any], interval_minutes: int = 30):
    ...
    return await supabase_bg_service.schedule_task(task_data, first_check_time, "normal") 