...
class SupabaseBackgroundService:
    ...
    async def get_task_logs(
        ...
        query = self.supabase.table('task_logs').select('*').order('created_at', desc=True).limit(limit)
    ...
    async def get_system_health(self) -> Dict[str, Any]:
        ...
        has_old_messages = any(stat.oldest_msg_age_seconds > 3600 for stat in queue_stats)  # 1 hour
