...
class FarmAutomationService:
    ...
    async def schedule_irrigation_cycle(
        ...
        logger.info(f"Scheduling irrigation cycle for shelf {shelf_id}, duration: {duration_minutes}min")
    ...
    async def schedule_light_cycle(
        ...
        logger.info(f"Scheduling light cycle for shelf {shelf_id}, duration: {light_hours}h")
        return await self.bg_service.queue_task(
            task_type="farm.light_schedule",
            payload=task_data["payload"],
            priority="normal",
    ...
    async def schedule_harvest_check(
        ...
        logger.info(f"Scheduling harvest check for schedule {schedule_id} on {check_date}")
        return await self.bg_service.queue_task(
            task_type="farm.harvest_check",
            payload=task_data["payload"],
            priority="low",
    ...
    async def trigger_automation_rule(
        ...
        logger.info(f"Triggering automation rule {rule_id}: {current_value} vs {trigger_value}")
        return await self.bg_service.queue_task(
            task_type="farm.automation_rule",
            payload=task_data["payload"],
            priority="high"
    ...
    async def schedule_all_farm_tasks(self, farm_id: str, user_id: str) -> Dict[str, List[str]]:
        ...
        try:
            ...
            monitoring_task = await self.schedule_sensor_monitoring(farm_id, user_id, 30)
...
async def handle_sensor_alert_threshold(
    ...
    try:
        ...
        if threshold_crossed:
            ...
            logger.info(f"Sensor threshold alert triggered for {reading_type}: {current_value} {condition} {threshold_value}")
            return task_id
