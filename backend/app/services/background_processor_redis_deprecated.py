...
class BackgroundProcessor:
    ...
    async def stop(self):
        ...
        all_tasks = self.workers + ([self.scheduler_task] if self.scheduler_task else [])
        if all_tasks:
    ...
    async def _process_task(self, task: BackgroundTask, worker_id: str):
        ...
        try:
            ...
            if task.result.retry_count <= task.max_retries:
                ...
                logger.info(f"Task {task.id} queued for retry {task.result.retry_count}/{task.max_retries}")
            else:
    ...
    async def _execute_task_with_timeout(self, func: Callable, args: List[Any], kwargs: Dict[str, Any], timeout: float):
