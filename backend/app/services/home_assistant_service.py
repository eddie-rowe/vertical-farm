...
class HomeAssistantService:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            if (self.settings.CLOUDFLARE_ACCESS_PROTECTED and 
                self.settings.CLOUDFLARE_SERVICE_CLIENT_ID and 
                self.settings.CLOUDFLARE_SERVICE_CLIENT_SECRET):
                ...
                logger.info("Configuring Home Assistant client with Cloudflare Access protection")
            ...
            async with self.client:
                ...
                if health.get("websocket", False):
                    logger.info("Home Assistant service initialized successfully with WebSocket support")
                else:
                    logger.warning("Home Assistant service initialized without WebSocket support (REST API only)")
    ...
    async def get_all_devices(self) -> List[Dict]:
        ...
        try:
            ...
            logger.info(f"Retrieved {len(relevant_entities)} relevant devices from Home Assistant")
    ...
    async def turn_on_device(self, entity_id: str, **kwargs) -> Dict:
        ...
        if domain not in ["light", "switch", "fan", "cover"]:
            raise HTTPException(
                status_code=400,
                detail=f"Cannot turn on device of type '{domain}'"
            )
    ...
    async def turn_off_device(self, entity_id: str) -> Dict:
        ...
        if domain not in ["light", "switch", "fan", "cover"]:
            raise HTTPException(
                status_code=400,
                detail=f"Cannot turn off device of type '{domain}'"
            )
    ...
    async def toggle_device(self, entity_id: str) -> Dict:
        ...
        if domain not in ["light", "switch", "fan"]:
            raise HTTPException(
                status_code=400,
                detail=f"Cannot toggle device of type '{domain}'"
            )
    ...
    async def control_irrigation_solenoid(
        ...
        if action == "open":
            ...
            raise HTTPException(
                status_code=400,
                detail="Invalid action or missing duration for pulse"
            )
    ...
    async def control_grow_light(
        ...
        if action == "off":
            ...
            raise HTTPException(status_code=400, detail="Invalid action for light control")
    ...
    async def get_available_services(self) -> Dict:
        ...
        try:
            ...
            for domain, domain_services in services.items():
                if domain in ["light", "switch", "fan", "cover", "climate", "homeassistant"]:
