...
class GrowAutomationService:
    ...
    async def initialize_grow_automation(self, grow_id: str, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Grow automation initialized for {grow_id} - using Supabase queues")
    ...
    async def _get_grow_device_assignments(self, grow_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            response = await self.supabase.rpc(
                'get_grow_device_assignments',
                {'grow_id_param': grow_id}
            ).execute()
    ...
    async def _apply_device_profile(
        ...
        try:
            # Get device profile
            profile_response = await self.supabase.table('grow_device_profiles').select('*').eq('id', profile_id).single().execute()
            ...
            matching_devices = [
                device for device in device_assignments 
                if device['device_type'] == device_type
            ]
            
            if not matching_devices:
                logger.warning(f"No {device_type} devices found for profile {profile_id}")
            ...
            for device in matching_devices:
                ...
                if 'temperature_trigger' in profile_config or 'humidity_trigger' in profile_config:
    ...
    async def _create_schedule_from_profile(
        ...
        try:
            ...
            if schedule_type == 'daily' and 'on_time' in config and 'off_time' in config:
                # Create ON schedule
                on_schedule = {
                    'grow_id': grow_id,
                ...
                on_response = await self.supabase.table('grow_automation_schedules').insert(on_schedule).execute()
                ...
                off_response = await self.supabase.table('grow_automation_schedules').insert(off_schedule).execute()
                ...
                response = await self.supabase.table('grow_automation_schedules').insert(schedule).execute()
    ...
    async def _create_conditions_from_profile(
        ...
        try:
            ...
            if 'temperature_trigger' in config:
                ...
                response = await self.supabase.table('grow_automation_conditions').insert(temp_condition).execute()
                if response.data:
            ...
            if 'humidity_trigger' in config:
                ...
                response = await self.supabase.table('grow_automation_conditions').insert(humidity_condition).execute()
                if response.data:
                    conditions.append(response.data[0])
    ...
    async def _execute_scheduled_action(self, schedule: Dict[str, Any]):
        ...
        try:
            ...
            logger.error(f"Error executing scheduled action for schedule {schedule['id']}: {e}")
    ...
    async def _execute_condition_action(self, condition: Dict[str, Any]):
        ...
        try:
            ...
            logger.error(f"Error executing condition action for condition {condition['id']}: {e}")
            # Try to log the error if execution_id exists
            if 'execution_id' in locals():
                await self._log_execution_complete(
                    execution_id,
    ...
    async def _log_execution_start(
        ...
        try:
            ...
            response = await self.supabase.table('grow_automation_executions').insert(execution_data).execute()
    ...
    async def _log_execution_complete(
        ...
        try:
            ...
            await self.supabase.table('grow_automation_executions').update(update_data).eq('id', execution_id).execute()
    ...
    async def get_grow_automation_status(self, grow_id: str, user_id: str) -> Dict[str, Any]:
        """Get automation status for a grow"""
        try:
            # Get active schedules
            schedules_response = await self.supabase.table('grow_automation_schedules').select('*').eq('grow_id', grow_id).eq('is_active', True).execute()
            
            # Get active conditions
            conditions_response = await self.supabase.table('grow_automation_conditions').select('*').eq('grow_id', grow_id).eq('is_active', True).execute()
            
            # Get recent executions
            executions_response = await self.supabase.table('grow_automation_executions').select('*').eq('grow_id', grow_id).order('started_at', desc=True).limit(10).execute()
