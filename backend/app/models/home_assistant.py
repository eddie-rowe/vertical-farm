...
class HomeAssistantStatusResponse(BaseModel):
    ...
    enabled: bool = Field(
        ..., description="Whether Home Assistant integration is enabled"
    )
    initialized: bool = Field(..., description="Whether the service is initialized")
    healthy: bool = Field(..., description="Overall health status")
    connected: bool = Field(
        default=False,
        description="Whether connected to Home Assistant (frontend compatibility)",
    )
...
class DeviceAssignmentRequest(BaseModel):
    ...
    def validate_assignment_level(self) -> "DeviceAssignmentRequest":
        ...
        if assigned_levels != 1:
            raise ValueError(
                "Device must be assigned to exactly one level: shelf, rack, row, or farm"
            )
...
class HomeAssistantConfigRequest(BaseModel):
    ...
    is_default: bool = Field(
        default=False, description="Whether this should be the default configuration"
    )
    ...
    def validate_local_url(cls, v: str | None) -> str | None:
        ...
        if v is not None:
            if not v.startswith(("http://", "https://", "ws://", "wss://")):
                raise ValueError(
                    "Local URL must start with http://, https://, ws://, or wss://"
                )
            return v.rstrip("/")
        return v
...
class HomeAssistantConfigResponse(BaseModel):
    ...
    cloudflare_enabled: bool = Field(
        ..., description="Whether Cloudflare Access is enabled"
    )
    is_default: bool = Field(
...
class ImportedDeviceResponse(BaseModel):
    ...
    is_assigned: bool = Field(
        default=False, description="Whether device is assigned to a farm location"
    )
