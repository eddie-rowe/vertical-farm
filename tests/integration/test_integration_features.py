...
class IntegrationTester:
    ...
    async def test_database_connection(self):
        ...
        try:
            async with self.session.get(f"{self.base_url}/api/v1/test/db-connection") as response:
                if response.status == 200:
                    data = await response.json()
                    ...
                    self.log_test("Database Connection + Caching", False, f"HTTP {response.status}")
                    return False
        except Exception as e:
    ...
    async def test_query_caching_performance(self):
        ...
        try:
            ...
            async with self.session.get(f"{self.base_url}/api/v1/test/db-query") as response:
                first_duration = time.time() - start_time
                if response.status != 200:
                    self.log_test("Query Caching Performance", False, f"HTTP {response.status}")
                    return False
    ...
    async def test_http_cache_headers(self):
        ...
        try:
            async with self.session.get(f"{self.base_url}/api/v1/test/cache-test") as response:
                if response.status == 200:
                    ...
                    self.log_test("HTTP Cache Headers", False, f"HTTP {response.status}")
                    return False
        except Exception as e:
    ...
    async def test_background_task_submission(self):
        ...
        try:
            ...
            async with self.session.post(
                f"{self.base_url}/api/v1/test/background/submit-task",
                json=task_data
            ) as response:
                if response.status in [200, 201]:
                    data = await response.json()
                    task_id = data.get('task_id', 'unknown')
                    self.log_test("Background Task Submission", True, f"Task ID: {task_id}")
                    return True
                else:
                    self.log_test("Background Task Submission", False, f"HTTP {response.status}")
                    return False
        except Exception as e:
    ...
    async def test_queue_statistics(self):
        ...
        try:
            async with self.session.get(f"{self.base_url}/api/v1/test/background/queue-stats") as response:
                if response.status == 200:
                    data = await response.json()
        ...
        try:
            ...
            async with self.session.post(
                ...
                if response.status in [400, 503, 422]:
                    ...
                    self.log_test("Home Assistant Error Handling", True, f"Graceful error: {error_msg}")
                    return True
                else:
                    self.log_test("Home Assistant Error Handling", False, f"Unexpected status: {response.status}")
                    return False
        except Exception as e:
    ...
    async def test_detailed_health_check(self):
        ...
        try:
            async with self.session.get(f"{self.base_url}/api/v1/test/health-detailed") as response:
                if response.status == 200:
                    data = await response.json()
                    services = data.get('services', {})
                    database_ok = services.get('database', {}).get('status') == 'healthy'
                    ...
                    self.log_test("Detailed Health Check", False, f"HTTP {response.status}")
                    return False
        except Exception as e:
