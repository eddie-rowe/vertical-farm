...
1. Open up Claude Code and run these slash commands in order to develop the feature:
   1. `/up` - Start local development environment
   2. `/plan ISSUE` - Analyze GitHub issue and create implementation plan
   3. `/dev ISSUE` - Develop feature from issue or description
   4. `/test` - Run comprehensive local testing
   5. `/validate ISSUE` - Validate implementation with Playwright
   6. `/deploy ISSUE` - Create PR and handle deployment
   7. `/pipeline PR` - Debug and fix CI/CD pipeline failures
   8. `/reflect [commits] [scope]` - Analyze patterns and improve workflows
   9. `/finalize ISSUE` - Close issue with documentation and logs
